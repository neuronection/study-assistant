...
Status: **COMPLETE (2026-08-31 — A, B, C, D, E all done; backend 784 · frontend 815 green)** · Phase: post-1.0 · **Ran after 54, before the 47–53 feature rounds** · As-built notes: C landed as domain batches (courses ×2, quiz, analytics, backup, ai/chat/sources, stragglers) instead of the original C1–C4 split — survey-driven, each batch committed with regenerated contract artifacts; final survey 0 untyped 200/201-JSON endpoints (binary downloads stay model-free by design; `context-vars`/`presets` are typed dict models). Vocabulary grew beyond the slice-A list with `ConceptRelation`, `ReviewFindingKind`, `StudyStatus`, `RecommendationKind`, `SpeedLabel`, `SpeedQuadrant`, `ItemFlag` as their domains were typed. E as-built: ErrorBanner/UndoNotice were already library-backed app wrappers (direct library imports keep the family drift audit clean — the audit's shadow rule accepts only direct library imports or shims themselves, so no unused error-banner/undo-notice shims were added); real adoption = BlockRenderer copy button → library `CopyButton` (shim), Spinner/EmptyState sweep on Jobs/Library/Practice/Scores + ExtractionView, and three shadowing app-glue files renamed (`AiGeneratedBadge`, `LibraryBreadcrumbs`, `MaterialUploadDropzone`) to fix audit regressions; audit clean over 335 files.
