    ...
    <div className="space-y-2">
      {lefts.map((left, index) => (
        <div key={index} className="flex items-center gap-2">
          ...
          >
            <BlockRenderer blocks={asBlocks(left)} actions={false} />
          </div>
          <select
        ...
        return (
          ...
          >
            ...
            <div className="flex-1">
              <BlockRenderer blocks={asBlocks(item?.label ?? '')} actions={false} />
            </div>
      ...
      {items.map((label, index) => (
        ...
        >
          <div className="min-w-40 flex-1">
              <BlockRenderer blocks={asBlocks(label)} actions={false} />
          </div>
          <div
    ...
    <p className="text-foreground flex flex-wrap items-center gap-1 text-sm leading-relaxed">
      {parts.map((part, index) => {
        ...
        if (!match) {
          return (
            <span key={index}>
              <BlockRenderer blocks={asBlocks(part)} actions={false} />
            </span>
          )
        }
