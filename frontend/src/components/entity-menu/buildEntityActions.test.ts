...
describe('buildEntityActions', () => {
  ...
  test('gates teachBack on the handler and passes entity + context', () => {
    const teachBack = vi.fn()
    const withTeachBack = buildEntityActions(
      makeSource(),
      'node',
      makeHandlers({ teachBack }),
      t
    )
    const action = withTeachBack[0].actions.find((a) => a.key === 'teachBack')
    expect(action).toBeDefined()
    action?.onSelect()
    expect(teachBack).toHaveBeenCalledWith(
      { id: '1', label: 'Node' },
      { courseId: 3, scopeNodeId: 5 }
    )

    const without = buildEntityActions(makeSource(), 'node', makeHandlers(), t)
    expect(
      without.flatMap((group) => group.actions.map((a) => a.key))
    ).not.toContain('teachBack')
  })
