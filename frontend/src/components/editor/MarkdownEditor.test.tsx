...
describe('MarkdownEditor', () => {
  ...
  test('re-OCR shows the old-vs-new transcript diff when the new text arrives', async () => {
    const adapter = {
      create: vi.fn(async () => null),
      update: vi.fn(async () => undefined),
      reocr: vi.fn(async () => undefined),
    ...
    rerender(
      <MarkdownEditor
        ...
        ariaLabel="Note body"
        drawings={[
          { id: 3, png_sha: 'abc', ocr_markdown: 'first text', ocr_job_id: 77 },
        ]}
        drawingAdapter={adapter}
      />
    )
    rerender(
      <MarkdownEditor
        ...
        ariaLabel="Note body"
        drawings={[
          { id: 3, png_sha: 'abc', ocr_markdown: 'second text', ocr_job_id: null },
        ]}
        drawingAdapter={adapter}
      />
    )
    await waitFor(() => expect(screen.getByText('changed')).not.toBeNull())
    const details = proseRoot().querySelector('details') as HTMLElement
    expect(details.getAttribute('open')).not.toBeNull()
    expect(details.textContent).toContain('first text')
    expect(details.textContent).toContain('second text')
    fireEvent.click(within(details).getByRole('button', { name: 'Show plain' }))
  ...
  test('editor body grows with the content; the toolbar stays above it (plan 62-G)', async () => {
    ...
    expect(root.closest('[class*="max-h"]')).toBeNull()
  ...
  test('broken latex offers the AI fix affordance, valid math does not (plan 64-D)', async () => {
    const { unmount: unmountBroken } = render(
      <MarkdownEditor
        value={'broken $\\frac{1}{$ here'}
        onChange={vi.fn()}
        ariaLabel="Note body"
      />
    )
