...
import { validateMathSource } from '@/lib/blockGate'
...
function MathView({ node, updateAttributes, selected, editor }: ReactNodeViewProps<HTMLElement>) {
  ...
  const validateMath = useCallback(
    async (text: string) => validateMathSource(text, display),
    [display]
  )
    ...
    <NodeViewWrapper as="span">
      ...
      >
        ...
        {diagnostic !== null && editor.isEditable ? (
          <button
            type="button"
            className="text-danger ml-1 inline-flex items-center gap-0.5 align-baseline text-[11px] underline"
            onClick={() => {
              setFixOpen(true)
              setEditing(true)
            }}
