...
export function mentionsToMarkdownLinks(md: string, mentions: MentionRef[]): string {
...
export function mentionUrlTransform(url: string) {
...
export function mentionComponents(mentions: MentionRef[]): Components {
  return {
    a: ({ href, children }) => (
...
function TextBlockView({ block }: { block: TextBlock }) {
  ...
  const markdown = normalizeMathFences(
    mentions.length ? mentionsToMarkdownLinks(block.md, mentions) : block.md
  )
...
function CodeBlockView({ block, actions }: { block: CodeBlock; actions: boolean }) {
  return (
    <CodeSurface
      code={block.code}
      menu={actions ? <BlockActions block={block} className="right-9" /> : undefined}
    >
    ...
    <div className={cn('space-y-2', className)}>
      {blocks.map((block, i) => {
        switch (block.type) {
          case 'text':
            return (
              <ActionableBlock key={i} block={block} actions={actions}>
          ...
          case 'math':
            return (
              <ActionableBlock key={i} block={block} actions={actions}>
                <MathBlockView block={block as MathBlock} />
              </ActionableBlock>
            )
          case 'diagram':
            return (
              <ActionableBlock key={i} block={block} actions={actions}>
                <DiagramBlockView block={block as DiagramBlock} />
              </ActionableBlock>
            )
          case 'chart':
            return (
              <ActionableBlock key={i} block={block} actions={actions}>
                <ChartBlockView block={block as ChartBlock} />
              </ActionableBlock>
            )
          case 'image':
          ...
          case 'table':
            return (
              <ActionableBlock key={i} block={block} actions={actions}>
                <TableBlockView block={block as TableBlock} />
              </ActionableBlock>
            )
          case 'code':
            return <CodeBlockView key={i} block={block as CodeBlock} actions={actions} />
          case 'geo':
            return (
              <ActionableBlock key={i} block={block} actions={actions}>
                <GeoBlockView block={block as GeoBlock} />
              </ActionableBlock>
            )
          case 'drawing':
            return (
