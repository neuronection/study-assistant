    ...
    <>
      {layout === 'grid' ? (
        <motion.div
          ...
          initial="hidden"
          animate="show"
          className="grid grid-cols-[repeat(auto-fill,minmax(140px,1fr))] gap-4 p-2"
        >
          <AnimatePresence>
          {items.map((item) => {
            ...
            return (
              <motion.div
                ...
                onDragStart={
                  onDragStart === undefined ? undefined : (event) => onDragStart(event as unknown as React.DragEvent, item)
                }
                onClick={(event) => {
