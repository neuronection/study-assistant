...
describe('EntityItems reduced motion', () => {
  test('grid and list render identically under prefers-reduced-motion', async () => {
    await rendersIdenticallyUnderReducedMotion(() => (
      <EntityItems items={ITEMS} layout="grid" menuItems={menu} />
    ))
    await rendersIdenticallyUnderReducedMotion(() => (
      <EntityItems items={ITEMS} layout="list" menuItems={menu} />
    ))
