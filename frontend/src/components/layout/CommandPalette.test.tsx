...
describe('CommandPalette', () => {
  ...
  test('keyboard navigation selects and runs an action', async () => {
    ...
    expect(screen.getByText('Snap a region into note')).toBeInTheDocument()
  ...
  test('snap-region action opens the snap capture flow (plan 67 C)', async () => {
    listCourses.mockResolvedValue([])
    renderPalette()
    fireEvent.click(await screen.findByText('Snap a region into note'))
    await waitFor(() => expect(useCaptureStore.getState().snapOpen).toBe(true))
