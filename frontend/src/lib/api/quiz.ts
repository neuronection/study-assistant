...
export async function quizAnswerKey(activityId: number): Promise<QuizAnswerKeyEntry[]> {
  const response = await apiFetch(`/api/v1/quiz/activities/${activityId}/answer-key`)
  return json<QuizAnswerKeyEntry[]>(response)
}
