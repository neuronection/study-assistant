...
export async function getFolderLinks(id: number): Promise<FolderLinkInfo[]> {
  const response = await apiFetch(`/api/v1/folders/${id}/links`)
  return json<FolderLinkInfo[]>(response)
}
