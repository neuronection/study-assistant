...
function formatJobDateTime(iso: string | null): string {
      ...
      <div className="flex items-start gap-3 p-3">
        <div className="min-w-0 flex-1 space-y-1.5">
          ...
          <p className="text-muted-foreground text-[11px]">
            #{job.id} ·{' '}
            {t('jobs.startedWhen', { time: formatJobDateTime(job.started_at ?? job.created_at) })} ·{' '}
            {t('jobs.finishedWhen', { time: formatJobDateTime(job.finished_at) })}
          </p>
        </div>
