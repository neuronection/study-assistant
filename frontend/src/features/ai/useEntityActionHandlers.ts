...
import { createChatSession, createNote, createTeachBack } from '@/lib/api'
...
export function useEntityActionHandlers(options: {
  ...
  const teachBack = useCallback((_entity: EntityNode, context: EntityContext) => {
    void createTeachBack({
  ...
  return { ask, generate, writeNote, teachBack, addNote }
}
