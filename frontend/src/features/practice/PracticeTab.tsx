  ...
  const menu = (item: PracticeRow): ContextMenuItem[] => {
    if (item.kind === 'quiz' && item.quizId !== undefined) {
      ...
      return [
        ...
        {
          key: 'print',
          label: t('quiz.printPaper'),
          onSelect: () => {
            void navigate({
              to: '/quiz/$activityId',
              params: { activityId: String(quizId) },
              search: { from, print: 'paper' },
            })
          },
        },
        {
          key: 'printKey',
          label: t('quiz.printKey'),
          onSelect: () => {
            void navigate({
              to: '/quiz/$activityId',
              params: { activityId: String(quizId) },
              search: { from, print: 'key' },
            })
          },
        },
        {
  ...
  return (
    <div className="space-y-6">
      ...
      <SegmentedControl
        ...
        value={segment}
        onChange={(next) => setSegment(next as PracticeSegment)}
      />
        ...
        <div className="space-y-2">
          ...
          {(flashcards.data ?? []).map((card) => (
            ...
            >
              ...
              <span className="text-muted-foreground shrink-0 text-[11px]">
                {card.due_at
                  ? t('cards.dueWhen', { when: formatDate(card.due_at.slice(0, 10)) })
                  : t('cards.state.new')}
              </span>
            </StaggerItem>
          ))}
