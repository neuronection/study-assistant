...
vi.mock('@/lib/api', async (importOriginal) => {
  ...
  return {
    ...
    promoteNode: (...args: unknown[]) =>
      promoteNode(...(args as [number, number, Record<string, unknown>])),
  }
})
...
function renderSidebar(
  ...
  options: { courseOrigin?: string | null } = {}
) {
    ...
    component: () => (
      <QueryClientProvider client={client}>
        <NodeTreeSidebar
          courseId="9"
          currentId={currentId}
          courseOrigin={options.courseOrigin ?? null}
        />
      </QueryClientProvider>
    ),
  })
...
describe('NodeTreeSidebar', () => {
  ...
  test('scratchpad context menu offers promote and the dialog submits the fields', async () => {
    promoteNode.mockResolvedValue({ id: 12, title: 'Abstract Algebra' })
    renderSidebar(undefined, { courseOrigin: 'scratch' })
    await screen.findByText('Derivatives')

    fireEvent.contextMenu(screen.getByText('Derivatives'))
    fireEvent.click(
      screen.getByRole('menuitem', { name: /promote to course/i }))

    fireEvent.change(await screen.findByPlaceholderText('Course name'), {
      target: { value: 'Abstract Algebra' },
    })
    fireEvent.click(screen.getByRole('button', { name: 'Create course' }))

    await waitFor(() =>
      expect(promoteNode).toHaveBeenCalledWith(9, 4, {
  ...
  test('non-scratch courses keep the promote item out of the menu', async () => {
    renderSidebar(undefined, { courseOrigin: 'manual' })
    await screen.findByText('Derivatives')

    fireEvent.contextMenu(screen.getByText('Derivatives'))
    expect(
      screen.queryByRole('menuitem', { name: /promote to course/i })
