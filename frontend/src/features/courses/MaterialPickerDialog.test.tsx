...
describe('MaterialPickerDialog', () => {
  ...
  test('All materials view lists everything with fuzzy filtering', async () => {
    ...
    expect(await screen.findByText('Lecture 2')).toBeInTheDocument()
    const lectureTwo = screen.getByText('Lecture 2')
    const lectureOne = screen.getByText('Lecture 1')
    expect(lectureTwo.compareDocumentPosition(lectureOne) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy()
  })
