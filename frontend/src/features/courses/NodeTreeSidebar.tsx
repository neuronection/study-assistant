...
export function NodeTreeSidebar({
  ...
  const [promoteTarget, setPromoteTarget] = useState<NodeInfo | null>(null)
  ...
  const promote = useMutation({
    mutationFn: (input: {
      ...
      promoteNode(Number(courseId), input.nodeId, {
        title: input.title,
  ...
  const menuItems = (node: NodeInfo): ContextMenuItem[] => {
    ...
    if (!node.is_root) {
      ...
      if (courseOrigin === 'scratch') {
        items.splice(2, 0, {
          key: 'promote',
          label: t('scratchpad.promoteMenuItem'),
          onSelect: () => setPromoteTarget(node),
        })
      }
    }
    return items
  ...
  return (
    ...
    >
      ...
      {promoteTarget !== null ? (
        <PromoteCourseDialog
          nodeTitle={promoteTarget.title}
          busy={promote.isPending}
          error={promote.isError ? t('scratchpad.promoteFailed') : null}
          onConfirm={(input) =>
            promote.mutate({ nodeId: promoteTarget.id, ...input })
          }
