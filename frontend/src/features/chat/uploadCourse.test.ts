...
describe('resolveUploadCourse', () => {
  ...
  test('prefers the Scratchpad over Unsorted and other courses', () => {
    const scratch = { ...course(3, 'Scratchpad'), origin: 'scratch' as const }
    expect(
      resolveUploadCourse([course(1, 'Calculus'), course(2, 'Unsorted'), scratch])?.id
    ).toBe(3)
  })
