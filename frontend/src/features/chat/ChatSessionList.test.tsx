...
describe('ChatSessionList (library list, app actions)', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  test('selecting a chat navigates to its route', async () => {
    ...
    renderList()
    fireEvent.click(await screen.findByText('Chain rule chat'))
  ...
  test('filters chats by search', async () => {
    ...
    await screen.findByText('Chain rule chat')
  ...
  test('deleting the active sidepanel chat calls onNewChat', async () => {
    ...
    const row = (await screen.findByText('Chain rule chat')).closest('[role="listitem"]')
    expect(row).not.toBeNull()
    fireEvent.click(within(row as HTMLElement).getByRole('button', { name: 'Delete chat' }))
  ...
  test('renames a chat from its row action', async () => {
    listChatSessions.mockResolvedValue(SESSIONS)
    renameChatSession.mockResolvedValue({ ...SESSIONS[0], title: 'Renamed' })
    renderList()
    const row = (await screen.findByText('Chain rule chat')).closest('[role="listitem"]')
    expect(row).not.toBeNull()
    fireEvent.click(within(row as HTMLElement).getByRole('button', { name: 'Rename chat' }))
  ...
  test('deletes a chat from its row action', async () => {
    listChatSessions.mockResolvedValue(SESSIONS)
    deleteChatSession.mockResolvedValue({ deleted_item_id: 4 })
    renderList()
    const row = (await screen.findByText('Chain rule chat')).closest('[role="listitem"]')
