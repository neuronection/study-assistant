...
> = {
  ...
  node: { icon: Network, labelKey: 'chat.attach.tabNodes' },
}
...
type TabKey = 'materials' | 'notes' | 'quizzes' | 'exercises' | 'nodes' | 'courses' | 'upload'
...
const TABS: {
  ...
  { key: 'nodes', icon: Network, titleKey: 'chat.attach.tabNodes' },
...
export function AttachMenu({
  ...
  const nodes = useQuery({
    queryKey: ['attach', 'nodes', courseId],
    queryFn: () => courseTree(courseId as number),
    enabled: tab === 'nodes' && courseId !== null,
  })

  const flattenNodes = (entries: NodeInfo[], path: string[]): Row[] =>
    entries.flatMap((entry) => [
      ...
      ...flattenNodes(entry.children ?? [], [...path, entry.title]),
    ])
