    ...
    createRoute({
      ...
      component: () => (
        <QueryClientProvider client={client}>
          <StudyChatProvider sessionId={props.sessionId ?? null}>
            <ChatPanel
...
describe('ChatPanel', () => {
  ...
  test('renders persisted tool calls as collapsible cards', async () => {
    ...
    renderPanel({ sessionId: 4 })
    const toggle = await screen.findByRole('button', { name: /SYMPY/ })
  ...
  test('tools button opens the tool catalog with only chat tools', async () => {
    ...
    expect(
      screen.getByText(
        'Numeric evaluation of an arithmetic expression. Chat answers.',
      ),
    ).toBeInTheDocument()
  ...
  test('pendingSend send posts through the transport with stripped attachments and refetches the transcript (plan 61-A)', async () => {
    listChatSessions.mockResolvedValue([SESSION])
    listChatMessages.mockResolvedValue([
      { id: 1, role: 'user', markdown: 'Summarize this', citations: [], grounded: null },
    ])
    getChatContext.mockResolvedValue({
      session_id: 4,
      course_id: null,
    ...
    renderPanel({ sessionId: 4 })
    await screen.findByPlaceholderText('Ask about your material…')
    act(() => {
      useChatStore.getState().queueSend({
        content: 'Summarize this',
        attachments: [{ kind: 'material', id: 3, title: 'rules.txt' }],
        mode: 'send',
      })
    })
    await waitFor(() =>
      expect(sendChatMessage).toHaveBeenCalledWith(4, 'Summarize this', [
        { kind: 'material', id: 3 },
      ]),
    )
    ...
    expect(await screen.findByText('Summarize this')).toBeInTheDocument()
  })

  test('pendingSend prefill seeds the composer and attachments without sending (plan 61-A)', async () => {
    listChatSessions.mockResolvedValue([SESSION])
    listChatMessages.mockResolvedValue([])
    getChatContext.mockResolvedValue({
      session_id: 4,
    ...
    await waitFor(() => expect(input.value).toBe('> quoted passage\n\n'))
...
describe('ChatPanel message actions (plan 40B)', () => {
  ...
  test('multiple proposals on one message render stacked cards (plan 66-A)', async () => {
    baseMocks()
    listChatMessages.mockResolvedValue([
      userMessage(),
      {
