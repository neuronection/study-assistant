...
describe('NewTextFileDialog', () => {
  ...
  test('save persists edits to the created material and stays open', async () => {
    ...
    await waitFor(() =>
      expect(onSave).toHaveBeenCalledWith({
        ...
        description: '',
        drawings: [],
        state: expect.objectContaining({ materialId: 7 }),
      })
    )
  ...
  test('description entered on the Description tab rides along with the create', async () => {
    const onCreate = vi.fn().mockResolvedValue(editState({ content: 'body' }))
    render(
      <NewTextFileDialog
        defaultKind="md"
        onCreate={onCreate}
    ...
    await waitFor(() =>
      expect(onCreate).toHaveBeenCalledWith(
        expect.objectContaining({ filename: 'notes.md', description: 'Chapter 3 recap' })
      )
    )
  })

  test('editing the description after create enables Save and passes it through', async () => {
    const onCreate = vi.fn().mockResolvedValue(editState({ content: 'body' }))
    const onSave = vi
      .fn()
      .mockResolvedValue(editState({ content: 'body' }))
    render(
      <NewTextFileDialog
        defaultKind="md"
        onCreate={onCreate}
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Create' }))
    fireEvent.click(await screen.findByRole('tab', { name: 'Description' }))
    fireEvent.change(screen.getByLabelText('Description'), {
