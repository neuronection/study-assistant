...
export function NoteEditor({
  ...
  useEffect(() => {
    ...
    const target: SnapTarget = {
      ...
      insert: async (pngBase64) => {
        const updated = await addDrawing(noteId, [], pngBase64, false, null)
        setError(null)
        const known = new Set((note.data?.drawings ?? []).map((entry) => entry.id))
        const fresh = updated.drawings.find((entry) => !known.has(entry.id))
        await queryClient.setQueryData(['note', noteId], updated)
        ...
        await queryClient.invalidateQueries({ queryKey: ['notes'] })
        if (fresh !== undefined) {
          editorApi.current?.insertDrawing(fresh.id)
        }
        return fresh?.id ?? null
      },
      remove: async (drawingId) => {
        await deleteDrawing(noteId, drawingId)
        editorApi.current?.removeDrawing(drawingId)
        setError(null)
        await queryClient.invalidateQueries({ queryKey: ['note', noteId] })
        await queryClient.invalidateQueries({ queryKey: ['notes'] })
      },
    ...
    return () => {
      if (useCaptureStore.getState().snapTarget === target) {
  ...
  const titleForm = (
    ...
    >
      <input
        className="bg-transparent w-full text-base font-semibold outline-none focus:border-b focus:border-border"
  ...
  const autosaveChip =
    autosave.dirty && autosave.status !== 'saving' ? (
      <button
        type="button"
        aria-label={t('notes.saveNow')}
        title={t('notes.saveNow')}
        onClick={() => autosave.saveNow()}
        className="text-muted-foreground hover:text-foreground rounded-full px-2 py-0.5 text-[11px] underline decoration-dotted underline-offset-2"
      >
      ...
      <span
        role="status"
        aria-live="polite"
        className="text-muted-foreground rounded-full px-2 py-0.5 text-[11px]"
      >
        {t(`notes.autosave.${autosave.status}`)}
      </span>
    ) : null
  ...
  const editorBlock = (
    <>
      <MarkdownEditor
        ...
        onSnapRegion={() => useCaptureStore.getState().openSnap()}
      />
  ...
  if (onClose === undefined) {
    const emptyState =
      markdown === '' && (data.drawings ?? []).length === 0 ? (
        <div
          data-testid="note-empty-state"
          className="border-border bg-subtle/40 text-muted-foreground space-y-1 rounded-md border border-dashed px-3 py-2.5 text-xs"
        >
          <p>{t('notes.emptyState.quote')}</p>
          <p>{t('notes.emptyState.math')}</p>
          <p>{t('notes.emptyState.ai')}</p>
        </div>
      ) : null
    return (
      <>
        <StudyPaneHeader
          ...
          primary={
            <>
              <ReadAloudButton markdown={markdown} />
              <PopoverMenu
          ...
          onClose={onRequestClose !== undefined ? requestClose : undefined}
          closeLabel={closeLabel}
        />
        <div className="flex min-h-0 flex-1 flex-col space-y-4 pt-3">
          {emptyState}
          {recoveryCard}
          {conflictCard}
          {actionResultCard}
