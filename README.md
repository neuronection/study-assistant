...
It is **beta** software, built for technical students first.
...
- **Import from the web** — paste a URL and the page becomes a standard markdown
  material (provenance recorded), searchable and usable as AI context like any other.
...
- **Tags & favorites** — tag and ⭐-star materials; filter the library by star or
  tag; both survive course export/import.
...
- **Quick capture** — `Ctrl/Cmd+Shift+K` from anywhere: jot a markdown thought
  into your scratchpad with one keystroke, undo or open it from the toast; the
  `Ctrl/Cmd+K` command palette jumps anywhere, runs creates and full-text search.
...
- **Human-in-the-loop proposals** — the tutor works through cards you approve:
  **edit or append to your notes and materials (reviewed as a diff before anything
  applies)**, create notes/flashcards/materials/concepts, generate quizzes and
  exercises **from materials you name**, organize (move, tag, exam dates) and
  draft study plans; execution is revalidated at approve time.
- **It knows your library and your record** — `FIND` searches your own materials
  and notes mid-answer, and insight tools let the tutor see your mistakes,
  error patterns, plan and due cards ("what do I keep getting wrong?").
...
- **English, Ελληνικά, Deutsch.** The UI ships in three languages (Settings → General);
  technical protocol tokens (CALC, SYMPY, OCR, LaTeX) stay recognizable and missing
  strings fall back to English.

## Status & roadmap

`docs/STATUS.md` is the single source of truth for phase and module status. Headline
next steps: AI-compose quality & scale (plan 70 direction), golden OCR eval
...
**Study Assistant** is one of four connected, open-source (Apache-2.0) AI assistants
...
<table>
  ...
  <tr>
    <td width="50%" align="center" valign="top">
      ...
      <br>
      <a href="https://neuronection.com/en/study/"><strong>Study Assistant</strong> <sub>· this repo</sub></a>
