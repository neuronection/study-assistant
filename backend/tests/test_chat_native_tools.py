...
def make_native_app(tmp_path: Path, gateway: NativeGateway) -> Any:
...
def test_native_calc_round_trip(tmp_path: Path) -> None:
...
def test_native_sympy_round_trip(tmp_path: Path) -> None:
    gateway = NativeGateway(
        [
...
def test_native_resource_tool_round_trip(tmp_path: Path) -> None:
    gateway = NativeGateway(
        [
...
def test_native_cap_gate_uses_prompt_path_without_tools_cap(tmp_path: Path) -> None:
    gateway = NativeGateway(
        [
            "Let me check.\nCALC 2^10\n",
            "It is $1024$. [1]",
...
def test_native_math_budget_bounds_rounds(tmp_path: Path) -> None:
    gateway = NativeGateway(
        [
...
def test_native_tool_call_ws_event_emitted(tmp_path: Path) -> None:
    gateway = NativeGateway(
        [
...
def test_tool_call_events_reassemble_unindexed_stream_fragments() -> None:
    """Providers that omit `index` on streamed tool_call deltas produce
    broken merged tool_calls (split name/args, empty dicts) — the gateway
    must reassemble the raw fragments instead of dropping the call
    (user report: empty tutor replies on GPT-5-class proxy models)."""

    from app.ai.gateway import _tool_call_events
...
def test_chat_native_schemas_cover_all_tools() -> None:
    from app.ai.tools import CHAT_TOOL_CATALOG, QUIZ_TOOL
    ...
    for schema in schemas:
        ...
        for name, prop in parameters["properties"].items():
            assert prop["type"] in valid_types, (schema["function"]["name"], name)
            if prop["type"] == "array":
                assert "items" in prop, (schema["function"]["name"], name)
...
def test_native_tool_unsupported_degrades_to_prompt_grammar(tmp_path: Path) -> None:
