...
def test_fix_mermaid_preset_runs_and_reports_diagnostic(
    client: tuple[TestClient, ScriptedGateway, FastAPI],
) -> None:
    test_client, gateway, _app = client
    with test_client:
        ...
        response = test_client.post(
            ...
            json=transform_payload(
                text="flowchart LR\n    O((O)) -->|v| P((v: (a,b)))",
                preset="fix_mermaid",
                diagnostic="Parse error on line 2: Expecting 'SQE', got 'PS'",
            ),
        )
        assert response.status_code == 200, response.text
        job_id = int(response.json()["job_id"])
        ...
        assert body["status"] == "done", body
        prompt = "\n".join(str(message.content) for message in gateway.calls[0])
        assert "Renderer diagnostic" in prompt
        assert "Expecting 'SQE'" in prompt
        assert "Repair this mermaid diagram source" in prompt


def test_fix_mermaid_output_with_fences_is_repaired(
    client: tuple[TestClient, ScriptedGateway, FastAPI],
) -> None:
    test_client, gateway, _app = client
    with test_client:
        gateway.responses.append("```mermaid\nflowchart LR\n    A --> B\n```")
...
def test_fix_math_preset_rejects_delimiters() -> None:
    problems = validate_output("$$\\frac{1}{2}$$", "\\frac{1}{", "fix_math")
    assert "delimiters" in problems[0]
    assert validate_output("\\frac{1}{2}", "\\frac{1}{", "fix_math") == []


def test_diagnostic_reaches_prompt_via_write_mode(
    client: tuple[TestClient, ScriptedGateway, FastAPI],
) -> None:
    test_client, gateway, _app = client
    with test_client:
        gateway.responses.append("Fixed content.")
        response = test_client.post(
            "/api/v1/ai/editor/transform",
            json=transform_payload(
                preset="fix_math",
                text="\\frac{1}{",
                mode="transform",
                diagnostic="KaTeX parseError: Expected group after '{'",
            ),
        )
        assert response.status_code == 200, response.text
        body = wait_for_job(test_client, int(response.json()["job_id"]))
        assert body["status"] == "done", body
        prompt = "\n".join(str(message.content) for message in gateway.calls[0])
        assert "KaTeX parseError" in prompt


def test_fix_presets_combine_user_instruction_with_base_directive(
    client: tuple[TestClient, ScriptedGateway, FastAPI],
) -> None:
    test_client, gateway, _app = client
    with test_client:
        gateway.responses.append("flowchart TD\n    A --> B")
        response = test_client.post(
            "/api/v1/ai/editor/transform",
            json=transform_payload(
                text="flowchart LR\n    A --> B",
                preset="fix_mermaid",
                instruction="make the flow top-down instead",
                diagnostic="Parse error",
            ),
        )
        assert response.status_code == 200, response.text
        ...
        prompt = "\n".join(str(message.content) for message in gateway.calls[0])
        assert "Repair this mermaid diagram source" in prompt
        assert "Additional user request: make the flow top-down instead" in prompt
        assert body["result_md"] == "flowchart TD\n    A --> B"
