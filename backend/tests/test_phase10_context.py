...
def test_ai_tools_catalog(client: TestClient) -> None:
    ...
    node_context = mcp_body["tools"][11]
...
def test_preview_structure_section_covers_neighborhood(client: TestClient) -> None:
    course_id = int(client.post("/api/v1/courses", json={"title": "C"}).json()["id"])
    root = root_node(client, course_id)
    chapter = make_node(client, course_id, root, "Chapter A")
    sibling = make_node(client, course_id, root, "Chapter B")
    section = make_node(client, course_id, chapter, "Section A1")

    preview = client.post(
        "/api/v1/ai/context/preview",
        json={"course_id": course_id, "node_id": section, "scope": "node"},
    )
    assert preview.status_code == 200, preview.text
    nodes = preview.json()["stats"]["nodes"]
    node_ids = {entry["id"] for entry in nodes}
    assert {chapter, section} <= node_ids
    ...
    chapter_preview = client.post(
        "/api/v1/ai/context/preview",
        json={"course_id": course_id, "node_id": chapter, "scope": "node"},
    )
    chapter_nodes = chapter_preview.json()["stats"]["nodes"]
    assert {sibling, section} <= {entry["id"] for entry in chapter_nodes}
    assert "(sibling)" in chapter_preview.json()["rendered"]
