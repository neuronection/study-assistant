...
def test_material_list_reports_direct_link_counts(client: TestClient) -> None:
    course_id = make_course(client, "Counts")
    folder_id = make_folder(client, course_id, "Docs")
    direct = add_material(client, "direct.md", course_id)
    via_folder = add_material(client, "vias.md", course_id, folder_id=folder_id)
    ...
    assert counts[via_folder] == 0


def test_folder_list_reports_own_node_link_counts(client: TestClient) -> None:
    course_id = make_course(client, "Folder counts")
    parent_id = make_folder(client, course_id, "Parent")
    folder_id = make_folder(client, course_id, "Docs", parent_id=parent_id)
    node_id = make_node(client, course_id, "Node")
    folder_link = client.post(
        f"/api/v1/nodes/{node_id}/folder-materials", json={"folder_id": folder_id}
    )
    ...
    folders = client.get("/api/v1/folders", params={"course_id": course_id}).json()
    counts = {entry["id"]: entry["node_link_count"] for entry in folders}
    assert counts[folder_id] == 1
    assert counts[parent_id] == 0


def test_folder_links_endpoint_lists_own_placements(client: TestClient) -> None:
    course_id = make_course(client, "Folder links")
    folder_id = make_folder(client, course_id, "Docs")
    node_id = make_node(client, course_id, "Node")
    second_node = make_node(client, course_id, "Alpha")
    root_id = client.get(f"/api/v1/courses/{course_id}/tree").json()[0]["id"]
    folder_link = client.post(
        f"/api/v1/nodes/{node_id}/folder-materials",
        json={"folder_id": folder_id, "rationale": "lecture set"},
    )
    assert folder_link.status_code == 201
    root_link = client.post(
        f"/api/v1/courses/{course_id}/folder-materials",
        json={"folder_id": folder_id},
    )
    assert root_link.status_code == 201

    links = client.get(f"/api/v1/folders/{folder_id}/links")
    assert links.status_code == 200
    body = links.json()
    assert len(body) == 2
    assert body[0]["node_id"] == root_id
    assert body[0]["is_course_level"] is True
    assert body[1]["node_id"] == node_id
    assert body[1]["is_course_level"] is False
    assert body[1]["breadcrumb"][-1] == {"id": node_id, "title": "Node"}
    assert body[1]["course_id"] == course_id
    assert body[1]["auto_assigned"] is False
    assert body[1]["rationale"] == "lecture set"
    assert all(entry["owner_title"] for entry in body)
    assert second_node not in {entry["node_id"] for entry in body}

    empty = make_folder(client, course_id, "Empty")
    assert client.get(f"/api/v1/folders/{empty}/links").json() == []
    missing = client.get("/api/v1/folders/999999/links")
    assert missing.status_code == 404
