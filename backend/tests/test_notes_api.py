...
def test_strokeless_snapshot_drawing_skips_ocr() -> None:
    client = make_client([])
    with client:
        note_id = create_note(client, "Snapshots")
        added = client.post(
            f"/api/v1/notes/{note_id}/drawings",
            json={
                "strokes": [],
        ...
        jobs = client.get("/api/v1/jobs", params={"type": "drawing_ocr"}).json()
        assert jobs == []
        ...
        assert updated.status_code == 200
        assert updated.json()["drawings"][0]["ocr_job_id"] is None

        deleted = client.delete(f"/api/v1/notes/{note_id}/drawings/{drawing['id']}")
