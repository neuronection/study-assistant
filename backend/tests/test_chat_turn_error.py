...
def wait_for_condition(
    ...
    while time.monotonic() < deadline:
        if condition():
            return
        time.sleep(0.05)
    detail = f"; last observed: {describe()}" if describe is not None else ""
    ...
    with test_client:
        ...
        try:
            ...
            def job_state() -> str:
                db.expire_all()
                job = db.get(Job, job_id)
                return "missing" if job is None else str(job.status)

            wait_for_condition(job_failed, describe=job_state)
            job = db.get(Job, job_id)
