...
def test_quiz_answer_key_endpoint(quiz_client: TestClient) -> None:
    created = quiz_client.post(
        "/api/v1/quiz/generate", json={"count": 3, "course_id": make_course(quiz_client)}
    )
    assert created.status_code == 201, created.text
    ...
    key = quiz_client.get(f"/api/v1/quiz/activities/{activity_id}/answer-key")
    assert key.status_code == 200, key.text
    entries = key.json()
    assert [entry["type"] for entry in entries] == ["single", "truefalse", "equation"]
    assert entries[0]["key"] == "A"
    assert entries[1]["key"] == "True"
    assert entries[2]["key"] == "2*x*sin(x) + x**2*cos(x)"

    missing = quiz_client.get("/api/v1/quiz/activities/999999/answer-key")
