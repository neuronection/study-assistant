...
@router.post("/teach-back", response_model=ExerciseOut, status_code=201)
def create_teach_back_exercise(
    body: TeachBackIn, session: Session = Depends(get_session)
) -> ExerciseOut:
    from ..services.study.teachback import create_teach_back

    profile = ensure_default_profile(session)
    try:
        exercise = create_teach_back(
            session,
        ...
        raise HTTPException(status_code=422, detail=str(error)) from error
    session.commit()
    return _exercise_out(exercise, 1)
