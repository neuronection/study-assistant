...
class ExtractionDiffOut(BaseModel):
    base: str
    target: str
    additions: int
    deletions: int
...
def _resolve_extraction_ref(
    ...
    try:
        ...
        raise HTTPException(
            status_code=422, detail="version ref must be a version or 'current'"
        ) from None
    row = rows_by_version.get(version)
    if row is None:
        raise HTTPException(status_code=404, detail="extraction version not found")
    return row.markdown, f"v{version}"


@router.get("/{material_id}/extractions/diff", response_model=ExtractionDiffOut)
def diff_extraction_versions(
    material_id: int,
    request: Request,
    ...
    if not rows_by_version:
        raise HTTPException(status_code=404, detail="material has no extractions")
    base_text, base_label = _resolve_extraction_ref(rows_by_version, from_ref)
    target_text, target_label = _resolve_extraction_ref(rows_by_version, to_ref)
    result = unified_text_diff(base_text, target_text, base_label, target_label)
    return ExtractionDiffOut(
        base=from_ref,
        target=to_ref,
...
def list_materials(
    ...
    if materials:
        for material_id, count in session.execute(
            select(MaterialLink.material_id, func.count())
            .where(MaterialLink.material_id.in_([m.id for m in materials]))
            .group_by(MaterialLink.material_id)
        ).all():
            link_counts[material_id] = count
    return [
        _to_out(
            material,
            has_extraction=material.id in extraction_ids,
            link_count=link_counts.get(material.id, 0),
        )
...
def import_url_material(
    ...
    _URL_IMPORTS_IN_FLIGHT.add(profile.id)
    try:
        try:
            content = perform_fetch(
                body.url.strip(),
...
def update_material(
    ...
    try:
        ...
        if "description" in body.model_fields_set:
            material = service.set_description(material, body.description)
        if "tags" in body.model_fields_set and body.tags is not None:
            material = service.set_tags(material, body.tags)
        if "starred" in body.model_fields_set and body.starred is not None:
            material = service.set_starred(material, body.starred)
    except ValueError as error:
        raise HTTPException(status_code=422, detail=str(error)) from error
    session.commit()
    return _to_out(
