...
from ..services.content.notes import (
    NOTE_VERSION_CAP,
    NoteBodyError,
    normalize_tags,
    save_note_body,
...
def update_note(
    ...
    if body.base_updated_at is not None:
        ...
        if utc_naive(base) != utc_naive(note.updated_at):
    ...
    if body.body_md is not None:
        try:
            save_note_body(
                ...
                note,
                body.body_md,
                cause="manual" if body.force_version else "autosave-coalesced",
    ...
    if body.tags is not None:
        note.tags = normalize_tags(body.tags)
    note.search_text = note_search_text(note)
    session.commit()
    return _note_detail(session, note)
...
class NoteVersionDiffOut(BaseModel):
    base: str
    target: str
    additions: int
    deletions: int
...
def _resolve_note_ref(
    note: Note, ref: str, versions_by_id: dict[int, NoteVersion]
) -> tuple[str, str]:
    if ref == "current":
        return blocks_md(note.body), "current"
    try:
        version_id = int(ref)
    except ValueError:
        raise HTTPException(
            status_code=422, detail="version ref must be an id or 'current'"
        ) from None
    version = versions_by_id.get(version_id)
    if version is None:
        raise HTTPException(status_code=404, detail="version not found")
    return blocks_md(version.body), f"#{version.id}"


@router.get("/{note_id}/versions/diff", response_model=NoteVersionDiffOut)
def diff_note_versions(
    ...
    versions_by_id = {version.id: version for version in rows}
    base_text, base_label = _resolve_note_ref(note, from_ref, versions_by_id)
    target_text, target_label = _resolve_note_ref(note, to_ref, versions_by_id)
    result = unified_text_diff(base_text, target_text, base_label, target_label)
    return NoteVersionDiffOut(
        base=from_ref,
        target=to_ref,
...
def restore_note_version(
    ...
    if version is None or version.note_id != note_id:
        raise HTTPException(status_code=404, detail="version not found")
    snapshot_note(session, note, "restore", force=True)
