...
def _proposal_valid(
    ...
    if not PROPOSAL_FENCE_RE.search(output):
        return []
    problems = validate_proposal_text(output)
    problems.extend(
        validate_proposal_context(
            output,
...
def _sources_cited_when_search_used(
    output: str, params: dict[str, Any], context: dict[str, Any]
) -> list[Violation]:
    sources = [str(url) for url in context.get("search_sources") or [] if str(url)]
    if not sources:
        return []
    ...
    for url in sources:
        ...
        if (domain and domain in lowered) or (registrable and registrable in lowered):
            return []
    return [
        Violation(
            "sources_cited_when_search_used",
            "the turn used web SEARCH/FETCH but the answer cites none of the "
            "retrieved sources — mention the URLs (or their domains) you relied on",
        )
    ]
...
VALIDATORS: dict[str, Validator] = {
    ...
    "sources_cited_when_search_used": _sources_cited_when_search_used,
}
...
CHAT_ANSWER_CONTRACT = [
    ...
    Constraint("sources_cited_when_search_used", advisory=True),
