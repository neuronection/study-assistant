"""Chat-turn graph (plan 10 Phase 5, ADR-0008; the only chat engine since
plan 72-A removed the legacy loop).

Implements the chat turn as a checkpointed StateGraph:
    ...
    for kind, argument in tool_calls:
        ...
        if kind == "READ":
            ...
            if state["find_used"] >= MAX_FIND_ROUNDS:
                results.append(
                    f"FIND {argument} -> error: FIND budget for this "
                    "turn is spent; answer from what you already have"
                )
                continue
            state["find_used"] += 1
            content = deps.service._find_local(argument, deps.chat_session, registry)
            results.append(f"FIND {argument} -> {content}")
            tool_calls_seen.append(
                _tool_entry(
                    deps,
                    "FIND",
            ...
            if state["search_used"] >= MAX_SEARCH_ROUNDS:
                results.append(
                    f"SEARCH {argument} -> error: SEARCH budget for this "
                    "turn is spent; answer from what you already have"
                )
                continue
            state["search_used"] += 1
            content, urls = deps.service._search_web(argument)
            sources = list(state["search_sources"])
            for url in urls:
                if url not in sources:
                    sources.append(url)
            state["search_sources"] = sources
            results.append(f"SEARCH {argument} -> {content}")
            tool_calls_seen.append(
                _tool_entry(
                    deps,
                    "SEARCH",
            ...
            if state["fetch_used"] >= MAX_FETCH_ROUNDS:
                results.append(
                    f"FETCH {argument} -> error: FETCH budget for this "
                    "turn is spent; answer from what you already have"
                )
                continue
            state["fetch_used"] += 1
            content = deps.service._fetch_url(argument)
            if not content.startswith("error:") and argument not in state["search_sources"]:
                state["search_sources"].append(argument)
            results.append(f"FETCH {argument} -> {content}")
            tool_calls_seen.append(
                _tool_entry(
                    deps,
                    "FETCH",
            ...
            if state["quiz_used"] >= MAX_QUIZ_ROUNDS:
                ...
                continue
            state["quiz_used"] += 1
            result, quiz_entry = deps.service._handle_quiz_tool(
                deps.chat_session,
                argument,
                phase,
                tool_start_ms,
                deps.elapsed_ms() - tool_start_ms,
                deps.emit,
            )
            results.append(f"QUIZ -> {result}")
            tool_calls_seen.append(quiz_entry)
        else:
