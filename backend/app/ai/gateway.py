...
class LLMGateway:
    ...
    def speak(
        ...
        *,
        ...
        for resolved in chain:
            try:
                audio, mime, usage = self._invoke_speech(resolved, text, voice)
            except ProviderError as error:
                last_error = error
                continue
            self._ledger(
                task,
                resolved,
                f"[tts {len(text)} chars]",
                "",
                int((time.monotonic() - started) * 1000),
                usage=usage,
            )
            return SpeechResult(audio=audio, mime=mime, model=resolved.label)
        assert last_error is not None
        raise last_error
    ...
    def _invoke_speech(
        ...
        for attempt in range(self._retry_attempts):
            try:
                with httpx.Client(timeout=self._timeout, transport=self._transport) as client:
                    return speak_with(client, resolved, text, voice)
            except SpeechUnsupported as error:
                raise ProviderError(resolved, str(error)) from error
            except Exception as error:
                if attempt + 1 >= self._retry_attempts or not is_transient_error(error):
                    raise _provider_failure(resolved, error) from error
...
def _tool_call_events(message: AIMessage) -> Iterator[StreamChunk]:
    ...
    # streamed tool_call_chunks do not always merge into usable parsed
    # `tool_calls` (providers that omit `index` produce split entries with
    # empty args) — fall back to the raw fragments and reassemble them so
    # tool calls are never silently dropped
    usable = bool(calls) and all(
    ...
    if not usable:
        ...
        for chunk in chunks:
            ...
            name = chunk.get("name")
            if index is None:
                # fragments without an index continue the previous call;
                # a named fragment (or the first one) starts a new group
                index = len(order) if name or not order else order[-1]
            if index not in grouped:
                grouped[index] = {"id": None, "name": "", "args": ""}
                order.append(index)
            entry = grouped[index]
            if name:
                entry["name"] = name
