...
from .services.platform.metrics import exam_status
...
GET_NODE_FLASHCARDS_DESC = (
    ...
    "due count, and up to 10 due card fronts. Never returns card backs."
)
GET_COURSE_MISTAKES_DESC = (
    "Recent wrong answers for the student's course: top error tags, the last "
    "few wrong answers (activity, question type, tags) and active discovered "
    "error patterns. Aggregates only."
)
GET_COURSE_PLAN_DESC = (
    "Study-plan summary for the student's course: exam date, days left, open "
    "plan items (oldest due first, overdue flagged), and the exam-readiness "
    "estimate with its honest not-enough-data state."
)
GET_COURSE_DUE_DESC = (
    "Due-flashcards overview for the student's course: total cards, due "
    "count, and up to 10 due card fronts. Never returns card backs."
)
...
def _due_card_fronts(
    session: Session, cards: list[Exercise], due_ids: set[int], limit: int = 10
) -> list[dict[str, Any]]:
    fronts: list[dict[str, Any]] = []
    for card in (candidate for candidate in cards if candidate.id in due_ids):
        if len(fronts) >= limit:
            break
        step = session.scalars(
            select(ExerciseStep)
            .where(ExerciseStep.exercise_id == card.id)
        ...
        fronts.append(
            {
                "id": card.id,
                "kind": card.kind,
                "front_md": blocks_to_md(step.prompt) if step else "",
            }
        )
    return fronts


def _get_node_flashcards(
    session: Session,
    node_id: int,
    ...
    if card_ids:
        ...
        due_ids = set(
            session.scalars(
                select(FsrsState.card_id).where(
                    FsrsState.card_id.in_(card_ids),
    ...
    return {
        "total": len(cards),
        "due": len(due_ids),
        "due_sample": _due_card_fronts(session, cards, due_ids),
    }
...
def _get_course_due(
    ...
    cards = _course_cards(session, course_id, profile)
    card_ids = [card.id for card in cards]
    due_ids: set[int] = set()
    if card_ids:
        now = datetime.now(UTC).replace(tzinfo=None)
...
def _get_course_mistakes(
    ...
    rows = session.execute(
        select(Answer, Activity.title, Question.type)
    ...
    for answer, activity_title, question_type in rows:
            ...
            recent.append(
                {
                    ...
                    "at": answer.created_at.isoformat(),
                }
            )
    patterns = list(
        session.scalars(
            select(ErrorPattern)
            .where(
                ErrorPattern.is_active.is_(True),
...
def _get_course_plan(
    ...
    forecast = next(
        (
            entry
            for entry in exam_status(session, profile.id)
    ...
    return {
        ...
        "days_left": forecast.get("days_left") if forecast else None,
        "open_items": [
            {
                "id": item.id,
                "title": item.title,
        ...
        "overdue_count": sum(1 for item in items if item.due_date < today),
        "readiness": readiness,
    }
...
RESOURCE_TOOLS: list[dict[str, Any]] = [
    ...
    {
        "keyword": "NODE_MATERIALS",
        "name": "get_node_materials",
        "description": GET_NODE_MATERIALS_DESC,
        "arg": "node",
        "call": _get_node_materials,
    ...
    {
        "keyword": "NODE_CONCEPTS",
        "name": "get_node_concepts",
        "description": GET_NODE_CONCEPTS_DESC,
        "arg": "node",
        "call": _get_node_concepts,
    },
    {
        "keyword": "NODE_FLASHCARDS",
        "name": "get_node_flashcards",
        "description": GET_NODE_FLASHCARDS_DESC,
        "arg": "node",
        "call": _get_node_flashcards,
    },
    {
        "keyword": "MISTAKES",
        "name": "get_course_mistakes",
        "description": GET_COURSE_MISTAKES_DESC,
        "arg": "course",
        "call": _get_course_mistakes,
    },
    {
        "keyword": "PLAN",
        "name": "get_course_plan",
        "description": GET_COURSE_PLAN_DESC,
        "arg": "course",
        "call": _get_course_plan,
    },
    {
        "keyword": "DUE",
        "name": "get_course_due",
        "description": GET_COURSE_DUE_DESC,
        "arg": "course",
        "call": _get_course_due,
    },
]
