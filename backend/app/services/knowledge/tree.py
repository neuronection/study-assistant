...
class TreeService:
    ...
    def move_subtree_to_course(self, node_id: int, target_course: Course) -> TreeNode:
        node = self.get(node_id)
        if node.is_root:
            raise TreeError("the course root cannot be promoted")
        source_course_id = node.course_id
        if source_course_id == target_course.id:
            raise TreeError("node already belongs to this course")
        target_root = self.ensure_root(target_course.id)
        subtree_depth = self._subtree_depth(node)
        if target_root.depth + subtree_depth > MAX_DEPTH:
            raise TreeError(f"nodes may nest at most {MAX_DEPTH} levels")
        moved_ids = self.subtree_ids(node)
        if self._session.get_bind().dialect.name == "sqlite":
            self._session.execute(sa_text("PRAGMA defer_foreign_keys=ON"))
        node.parent_id = target_root.id
        node.course_id = target_course.id
        node.depth = target_root.depth + 1
        node.order_idx = self._next_order(target_root.id)
        self._session.flush()
    ...
    def _move_exclusive_content(
        self, source_course_id: int, moved_node_ids: list[int], target_course_id: int
    ) -> None:
        ...
        moved_folder_ids = set(
            self._session.scalars(
                select(MaterialFolderLink.folder_id).where(
                    MaterialFolderLink.node_id.in_(moved_node_ids)
                )
            )
        )
        remaining_folder_ids = set(
            self._session.scalars(
                select(MaterialFolderLink.folder_id).where(
                    MaterialFolderLink.course_id == source_course_id
                )
            )
        )
        exclusive_folders = moved_folder_ids - remaining_folder_ids
        for folder_id in exclusive_folders:
            folder = self._session.get(MaterialFolder, folder_id)
            if folder is None or folder.course_id != source_course_id:
                continue
            folder.course_id = target_course_id
            for material in self._session.scalars(
                select(Material).where(
                    Material.folder_id == folder_id,
                    Material.course_id == source_course_id,
                )
            ):
                material.course_id = target_course_id
        for material_id in moved_material_ids - remaining_material_ids:
            orphan = self._session.get(Material, material_id)
            if orphan is None or orphan.course_id != source_course_id:
                continue
            if orphan.folder_id is not None and orphan.folder_id not in exclusive_folders:
                continue
