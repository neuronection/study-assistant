...
class ContextBundle:
    ...
    def _nodes_section(self) -> str:
        ...
        for entry in self.nodes:
            label = f"[T{entry['id']}] {entry['title']}"
            relation = entry.get("relation")
            if relation:
                label += f" ({relation})"
            summary = " ".join((entry.get("summary") or "").split())[:80]
            if summary:
                label += f" — {summary}"
            lines.append(label)
        return "\n".join(lines)
    ...
    def _scope_nodes(self, node: TreeNode) -> list[dict[str, Any]]:
        ...
        if parent is not None:
            ...
            siblings = self._session.execute(
                select(TreeNode.id, TreeNode.title, TreeNode.summary)
                .where(
            ...
            for sibling_id, title, summary in siblings:
                nodes.append(
                    {
                        "id": sibling_id,
                        "title": title,
        ...
        for child_id, title, summary in children:
            nodes.append(
                {
                    "id": child_id,
                    "title": title,
