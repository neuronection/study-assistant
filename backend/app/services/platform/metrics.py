...
def answer_rows(
    ...
    if course_id is not None:
        ...
        statement = statement.join(Course, Activity.course_id == Course.id).where(
...
def daily_history(session: Session, profile_id: int) -> list[dict[str, Any]]:
    ...
    quizme = session.execute(
        select(QuizmeAnswer.correct, QuizmeAnswer.created_at)
...
def due_cards_count(
    ...
    if course_id is not None:
        ...
        unscheduled_statement = unscheduled_statement.join(
            Course, Exercise.course_id == Course.id
        ).where(Course.origin != CourseOrigin.SCRATCH.value)
...
READINESS_TREND_WINDOW = 10
MIN_ANSWERS_FOR_READINESS = 3
READINESS_TREND_DELTA = 0.1
TREND_SCORES = {"improving": 1.0, "flat": 0.6, "declining": 0.2}
...
def course_readiness(
    ...
    *,
    ...
    """Mastery-weighted readiness estimate (plan 53-B): a documented mix of
    coverage (engaged nodes), mastery (answer accuracy across the weakness
    matrix cells in scope) and recent trend (last-window vs prior-window
    accuracy). ``score`` is None when there are fewer than
    MIN_ANSWERS_FOR_READINESS answers in scope — no score theater."""
    ...
    weights = [max(1, int(cell["n"])) for cell in cells]
    accuracies = [float(cell["accuracy"]) for cell in cells]
    mastery = (
        sum(w * a for w, a in zip(weights, accuracies, strict=True)) / sum(weights)
        if weights
        else coverage
    )
    window = min(READINESS_TREND_WINDOW, max(2, n // 2))
    recent = rows[-window:]
    older = rows[-2 * window : -window]
    recent_acc = sum(1 for row in recent if row.correct) / len(recent)
    if older:
        older_acc = sum(1 for row in older if row.correct) / len(older)
        delta = recent_acc - older_acc
        trend = (
            "improving"
            if delta > READINESS_TREND_DELTA
            else "declining"
            if delta < -READINESS_TREND_DELTA
            else "flat"
        )
    ...
    score = round(
        ...
        * (
            ...
            + READINESS_WEIGHTS["trend"] * TREND_SCORES[trend]
        )
    )
    return {
...
def recommendations(
    ...
    for cell in weak_cells[:3]:
        ...
        # plan 53-D: a heavily-drilled cell that is still weak turns into a
        # teach-back suggestion — explain it instead of another drill round.
        if cell["n"] >= 6 and cell["skill"] != TEACHBACK_SKILL:
            recs.append(
                {
                    "kind": RecommendationKind.TEACHBACK,
                    "priority": round(cell["weakness_score"] * 50) + 10,
