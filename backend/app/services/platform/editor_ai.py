...
EDITOR_PRESETS: dict[str, str] = {
    ...
    "fix_mermaid": (
        "Repair this mermaid diagram source so it parses and renders. Keep the "
        "diagram type and every node, label, and edge. Output ONLY the corrected "
        "diagram source — no code fences, no explanations."
    ),
    "fix_math": (
        "Repair this LaTeX expression so KaTeX renders it without errors. Keep the "
        "mathematical meaning exactly. Output ONLY the corrected LaTeX — no "
        "delimiters, no explanations."
    ),
}
...
def validate_output(text: str, input_text: str, preset: str | None) -> list[str]:
    ...
    if preset == "fix_mermaid" and ("```" in stripped or stripped.startswith("$$")):
        problems.append("mermaid source must not contain code fences or math delimiters")
    if preset == "fix_math" and (stripped.startswith("$$") or stripped.endswith("$$")):
        problems.append("latex must not be wrapped in $$ delimiters")
    return problems
...
def _effective_instruction(instruction: str, preset: str | None) -> str:
    ...
    if preset_key is not None and preset_key.startswith("fix_"):
        base = EDITOR_PRESETS[preset_key]
        if effective:
            return f"{base} Additional user request: {effective}"
        return base
