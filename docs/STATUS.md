...
Single source of truth for what exists and what phase we are in. Update with
every change (see AGENTS.md).
...
the only engine — `ChatEngine`/`SA_CHAT_ENGINE`/setting/selection branch and
the ~650-line `answer_streaming` loop deleted; parity harness replaced by
graph-only contract tests; surfaced + fixed a latent graph bug where the
`DEGRADED` short-circuit finalized an empty answer instead of retrying with
the degraded grammar). **B:** chat prompts open with a compact
**course-structure block** — location breadcrumb, parent, siblings, children
as `[T#] title — summary` (≤8/group, ≤6 lines) — and every rendered node is
registered as a mention handle, so neighbors are addressable (`READ T#`,
resource tools). **C:** new **`NODE_MATERIALS`** chat resource tool (node's
placed materials; MCP server variant unchanged) and `READ` on nodes now
lists children + placed materials — tool results register handles (ADR-161),
so node → material → full-extraction drill-down works. **D:** proposal
`node_id` payloads are validated against the course tree **at creation**
(`proposal_valid` repair round via `prep.context["course_node_ids"]`;
`PROPOSAL_DOC` points at structure ids) — invented ids no longer die as
approve-time stale cards. **E:** generation prompts (`render_prompt`)
cover the neighborhood too — structure section gained parent/siblings with
relation labels + summaries, `NODES_CAP` 16 → 24. **F:** the chat attach
menu gains a **Course nodes** tab (flattened tree, summary/breadcrumb meta)
sending `kind:'node'` attachments. Residual finding: `tests/evals/` does not
exist — AGENTS.md's golden-set gate references a missing path. Backend
1,081 green (+11), ruff/mypy clean, `check-ai-alignment.sh` clean; frontend
1,162 green (+7), lint/typecheck/build/`pnpm i18n` green; 8/8 e2e specs
green. Docs: ai.md, features.md.

**Proposal-card review upgrade (2026-09-10, user-requested follow-up to plan
66):** every diff/create proposal card now shows **where it acts** — the
backend enriches payloads at creation (`resolve_proposal_target` in
`proposal_actions.py`: `target_kind`/`target_name` + `target_node_id`/
`target_node_path` course-rooted breadcrumb for placed payloads) and the card
renders a target row (kind chip · name · breadcrumb) under the header, with an
executed-material "Open the material" link (`result.material_id` now surfaced).
Edit/append diff reviews render **formatted by default** through the new family
library component **`MarkdownDiffView`** (`@neuronection/assistant-ui/markdown-
diff-view`, shim `components/ui/markdown-diff-view.ts`; released in
assistant-ui **0.32.0**, dependency bumped + workspace
`minimumReleaseAgeExclude` gained `|| 0.32.0`):
block-level markdown diff (fences/`$$` math stay intact) rendered through
chat-markdown so KaTeX/tables/headings read rendered, unchanged blocks shared
full-width, folds + change navigation; a **Formatted | Raw** toggle keeps the
source-precise `TextDiffView` one click away. Library side: `computeUnitDiff`
added to `text-diff-view` (engine generalized over string arrays;
`computeLineDiff` unchanged). Backend 1,082 green (+1 `target_info` test),
ruff/mypy clean; frontend 1,164 green (+2 net, diff suites re-pointed to the
...
branch deleted, the ~650-line `ChatService.answer_streaming` loop with them,
the `chat_turn` job handler raises an honest `JobError` if no engine exists
(no silent fallback), and the dual-engine parity harness is replaced by
graph-only tests asserting the same WS contract (delta/reasoning/coalescing
trace tests moved to the real-model SSE harness — scripted gateways cannot
feed LangChain messages-mode streaming). Removal surfaced + fixed a latent
graph bug: the `DEGRADED` short-circuit skipped the degraded-grammar retry
and finalized an empty answer — now routes back to `agent_round` without
counting a repair round. Backend 1,070 green (ruff/mypy clean; count flat —
parity harness traded for graph-only equivalents), `check-ai-alignment.sh`
clean, 8/8 e2e specs green against the real backend + mock provider.

**Plan 72 (AI sees the course tree) COMPLETE 2026-09-09 — see the entry
above.** Plan doc: `dev/plans/72-ai-tree-structure-context.md` (local-only).

**Plan 71 slices A–C — library placement references (2026-09-09,
user-approved follow-up; ADR-159; renumbered from 156 after plan 70 claimed
156–158):** course-tree placements are now visible in the Library. **A:**
`MaterialOut.link_count` + `FolderOut.node_link_count` (computed on read —
one grouped query per list over the unique-indexed link tables, so counts
cannot double-count; badge counts **direct** placements only, by design) and
`GET /folders/{id}/links` (`StructureService.folder_links` — the folder's
own placements with breadcrumbs/auto/rationale, mirroring the material
endpoint; the whole-subtree view stays in `delete-info`). OpenAPI
regenerated. **B:** small `Link2`+count badge (`LinkRefBadge`) on library
folders, `MaterialTile` and `MaterialRow` when placed (count > 0); counts
ride the existing `['materials']`/`['folders']` queries so current
invalidations refresh them. **C:** right-click **Linked locations** (single
selection, materials + folders) opens a read-only dialog over
`getMaterialLinks`/`getFolderLinks` — breadcrumb rows with Auto/via-folder
chips + rationale, deduped by node (direct wins over via-folder), click
navigates to the node workspace, honest empty state. i18n en/de/el complete
(`pnpm i18n` green). Backend 1,070 green (+3), ruff/mypy clean; frontend
1,157 green (+7), lint/typecheck/build green.

**Workspace overview — practice/ask pending state scoped to the clicked node
(2026-09-09, user-reported fix):** clicking **Practice** on one child card of the
course overview made **every** card's Practice/Ask buttons show the spinner —
`NodeWorkspace` passed its single shared `studyHere`/`askAbout` mutation
`isPending` boolean to all `ChildCard`s (generation itself was always correctly
targeted). Pending state is now per-node: the parent derives `studyTargetId`/
`askTargetId` from the mutation's `variables` while pending, so only the clicked
card spins — other cards' buttons briefly disable without a spinner to prevent a
second concurrent generation — and the header **Ask about** button plus the
Tutor tab's ask action scope the same way (spinner only when the pending ask
targets that node). Regression test locks per-card isolation. Frontend 1,154
green (+1), lint/typecheck/build green; backend untouched.

**Plan 66 COMPLETE — slices A–I: proposals, `FIND` search + insight tools
(2026-09-09, user-approved; ADR-144/145/146/147):** **A:** the chat proposal layer is now
registry-driven — `PROPOSAL_ACTIONS` (`app/ai/proposals.py`) pairs every action
with its pydantic payload model + prompt grammar line (`PROPOSAL_DOC` is
assembled, no hand-sync) and an `api_executed` flag; service executors moved
into an `EXECUTORS` dict (`services/platform/proposal_actions.py`),
completeness locked by test both ways. **Multi-proposal:** cap raised 1 → 3 —
`extract_proposals` returns up to three valid proposals, `finalize_turn`
persists one `chat_proposals` row each (frontend already mapped the list —
stacked independently-approvable cards). **B:** new content-edit actions
`edit_note` {note_id, new_body_md, reason?} and `append_note` {note_id,
markdown, heading?}: at proposal creation `capture_proposal_snapshot` stores
the target note's current markdown (`original_md`) in the payload — the model
never supplies originals; the ProposalCard renders the change as a
`TextDiffView` review (append previews the merged result), and approve-time
execution refuses when the note's body no longer matches the snapshot
(status `stale`, honest error on the card). Note save/extract-to-service:
new `services/content/notes.py` (`snapshot_note`/`save_note_body`/
`validate_note_drawing_blocks`/`utc_naive`) — the notes PATCH endpoint and the
new executors share one versioned path (`NoteVersion` rows, cause `ai-edit`). **C:** the same snapshot→diff→
approve→execute pattern now covers materials — `edit_material` {material_id,
new_markdown, reason?} and `append_material` {material_id, markdown, heading?}
snapshot the latest extraction's markdown (`snapshot="extraction_md"`),
execute through `MaterialsService.edit_extraction` (new extraction version +
rechunk + FTS sync + index-card reset), enqueue a `postprocess` job, and the
approve endpoint wakes the job runner (`POSTPROCESS_ACTIONS`); executors now
receive a `ProposalContext` (blobs) so service-level execution stays
constructor-honest. **D:** generation proposals are context-aware —
`generate_quiz`/`generate_exercise` payloads gain `material_ids`/`note_ids`
(≤10 each) + `instructions`, quiz-only `question_types` (closed vocabulary
moved to `core/vocab.py QUESTION_TYPES`, single-sourced for quizgen + API +
proposals) and `shuffle`; `validate_proposal_context` cross-checks those id
lists against the turn's offered mention refs inside the `proposal_valid`
contract (unoffered ids → repair round), and the frontend approve path hands
the full payload to `GenerateDialog` which pre-seeds context materials, note
chips (titles resolved async), instructions, quiz types and shuffle. **E:**
`generate_flashcards` {material_id?|note_id?, count} joins the open-dialog
generate actions (dialog pre-seeds the flashcards source picker), and two
create actions land: `create_material` {title, body_md, node_id?} executes
via `MaterialsService.create_text` + queue_ingest + AI-proposal node link
(card previews the body as markdown) and `create_concept` {name,
description?, node_id?} is an idempotent concept-create with optional node
coverage link. **F:** new `FIND` chat tool (`CHAT_TOOL_CATALOG` +
`TOOL_LINE_RE` + both engine dispatches, `MAX_FIND_ROUNDS = 2`) —
`_find_local` runs `hybrid_search` (materials) + note `search_text` matching
course-scoped, registers up to 10 titled handles into the mention registry so
follow-up `READ`s work in the same turn; foreign-course content never leaks.
Frontend tool card registry + i18n updated; tools-endpoint catalog tests
re-pointed. **G:** five new read-only resource tools — `NODE_CONCEPTS`
(promoted from MCP-only), `NODE_FLASHCARDS` (totals + due fronts, never
backs) and course-scoped `MISTAKES`/`PLAN`/`DUE` (wrong answers + error
tags + discovered patterns; plan items + exam date + readiness via the
shared `exam_status` reader; due overview) with a new `course` argument kind
resolved from the chat session; the MCP server exposes the same additions
and `RESOURCE_TOOL_KEYWORDS` gained all five keywords. **H+I:** organize and
planner proposals — `move_to_node` (placement-node moves for
notes/quizzes/exercises + material assign), `tag_note` (normalize+merge,
`normalize_tags` moved to the notes service), `set_exam_date`,
`generate_plan` (deterministic planner executed on approve; PlannerError →
stale with the honest message) and `add_plan_items` (manual items,
`PLAN_ITEM_KINDS` vocabulary from core/vocab.py, exactly-one-of
due_date/days_ahead); proposal payloads now round-trip through
`model_dump_json` so dates persist as ISO strings.
Backend 1,059 green (+22 over the round), ruff/mypy clean; frontend 1,097
green (+9), lint/typecheck/build green; family AI-alignment gate clean.
Docs: ai.md.

**Plan 65 A–D — selection-aware AI helper COMPLETE (2026-09-07, user-approved;
ADR-143; zero backend):** the ✨ `AiHelperPopover` now operates on **markdown
slices** instead of text-only selections. **A:** new
`components/editor/selectionCapture.ts` — `selectionToMarkdown` serializes the
selection slice (`doc.cut(from, to)` wrapped in a standalone doc node) through
the tiptap-markdown serializer + `decodeMarkdownFromSerialize`, so mermaid →
fence, math → delimiters, tables → pipes byte-identically to the editor's own
serialize output (verified by corpus tests over real editors; node selections
captured naturally via `selectedRange()`'s DOM fallback); `summarizeFragment`
counts diagrams/formulas/code blocks/tables/images/paragraphs (inline math and
drawing refs inside text blocks included, empty paragraphs skipped).
**B:** renderer validators deduped into `lib/blockGate.ts`
(`validateMermaidSource`/`validateMathSource`/`extractBlock`/`gateResult`) —
`CaMermaid`/`CaMath` dialogs and the helper now share one gate implementation.
**C:** the popover captures the selection on open and on every run; idle view
gains the `SelectionSummaryChip` (collapsed by default, keyboard-reachable
disclosure, expandable ≤280-char preview + line count) and the scope pill
upgrades to "Selection — diagram(s)/formulas/…"/"Selection — mixed (N blocks)";
a **Try to fix** entry is pinned above the presets when the selection contains
math/mermaid — it self-diagnoses through `blockGate`, runs tier-0
`repairMermaidSource` first (deterministic results land instantly as
"Fixed without AI" in the review diff), else the plan-64 `fix_*` preset with
the diagnostic attached; single math/mermaid-node captures are **gated on
Apply** (result must still parse as the same kind — failure disables Apply,
shows the renderer error, and offers "Insert as markdown anyway"); cross-kind
and mixed results apply as markdown with an explicit "Applies as markdown"
label. Apply replaces the captured range in one undo step; every transform
reviews as the standard diff (fixes included). Free-form ask runs over the
captured markdown for block selections. **D:** fidelity corpus extended
(heading+list+blockquote mixes, drawing refs, `$` inside code spans,
whole-doc byte-equality). Frontend 1,088 tests green (+49 over the plan-64
state), lint/typecheck/build green; backend untouched-green (1,038, ruff/mypy
clean). Also: a pre-existing BlockFixPanel test race (Apply clicked before the
async gate resolved) hardened — clicks now wait for enablement; 4 consecutive
full-suite runs stable.

**Plan 64 A–D — AI block fixer (2026-09-07, user-approved; ADR-142):** broken
mermaid/math blocks in the shared editor gain a ✨ **Fix with AI** flow. Backend:
new `fix_mermaid`/`fix_math` editor-transform presets + additive `diagnostic`
request field (renderer parse error → prompt; fence/`$$`-wrap validators; OpenAPI
regenerated; repair-round streaming now resets the job buffer on `editor_repair` —
previously published the failed attempt concatenated with the retry). Frontend:
transport adapter extracted to `useEditorTransformTransport` (reused by
`AiHelperPopover`); new `BlockFixPanel` (streams the fix, reviews as
`TextDiffView`, deterministic gate — `mermaid.parse` / KaTeX `throwOnError` must
pass or Apply is disabled); `MermaidDiagram` surfaces `onError`/`onSuccess`;
`CaMermaid`/`CaMath` dialogs host the panel and failed blocks show the ✨
affordance; tier-0 deterministic mermaid repairer (`lib/mermaidRepair.ts` — quotes
unquoted node labels with parens/colons, e.g. the m36 `P((v: (a,b)))` case) runs
before the model and renders as "Fixed without AI", with a free-form **Ask AI**
box (user instruction combined with the preset directive server-side) to steer or
re-run the fix. Reader stays read-only; Apply writes node attributes only after
gate + user action. Frontend 1,059 green, lint/typecheck/build green; backend
1,037 green, ruff/mypy clean. Remaining: commit + usage-doc pass.

**Plan 63 follow-up — mermaid renders serialize (2026-09-07, user-reported via
material 36):** flowcharts showed raw code in the read view (0/5 rendered; editor
flaky 1–4/5). Root causes: one extractor-authored invalid diagram (`P((v: (a,b)))`
— parse error everywhere) whose failure path swept **all** in-flight mermaid temp
containers globally (`sweepStrayErrorDiagrams`), cascading `addA11yInfo` null
crashes into every concurrent render, amplified by all diagrams rendering
concurrently (×StrictMode double-effects). `MermaidDiagram` now serializes renders
through a module-level promise chain and cleans up only its own temp container.
Live-verified: read view and editor agree — 4/5 diagrams render, the invalid one
shows raw source in both (extractor content bug, out of scope). Frontend 1,043
green, lint/typecheck/build green; backend untouched.

**Plan 63 COMPLETE — viewer block copy actions + math-viewer fidelity (2026-09-07,
user-approved; slices A→E, zero backend, ADR-140/141):** **A–C:** every
`BlockRenderer` surface (chat transcript, reading view, note history, quiz
stems/explanations, flashcard review, exercise hints, cheat sheets) gains a
hover/focus-revealed ⋯ **Copy options** menu — app-side `BlockActions` +
`blockActions.ts` vocabulary + `lib/clipboard.ts` (async clipboard, `execCommand`
fallback), composed on the family library's `ActionMenu` (ADR-140: promotion-shaped,
not promoted). Vocabulary: math → Copy LaTeX / Copy as Markdown; code → fenced block
next to the existing direct copy button; text → markdown; diagram/chart/table/geo →
mermaid / figure JSON / GFM table / JSXGraph script; non-copyable blocks unchanged.
Suppressed (`actions={false}`) inside clickable answer rows (quiz choices,
math-trainer chips, error-spot lines, matching/ordering labels) and the flashcard
print sheet. **D–E (user-reported `\overset` bug):** root cause — the native
extractor stored multi-line display math with `$$` sharing lines with content; the
editor scans text-level and renders it, but micromark's flow math needs bare fences,
so the tail rendered raw. Fix is viewer-side (ADR-141): new `normalizeMathFences`
(`markdownFidelity.ts`) canonicalizes display spans into `$$⏎content⏎$$` before
remark parsing — applied in `TextBlockView`, chat `TranscriptMessage`/streaming
`MarkdownSurface`, and `MarkdownPrintDoc`; single-line `$$x$$` alone on its line now
renders display-style like the editor. Stored markdown never rewritten. Verified
against the full material-84 payload (zero raw `$$` paragraphs). Known residual:
extractor damage in the same material (dangling latex around empty `$$` pairs)
renders raw in the editor too. Frontend 1,042 green, lint/typecheck/build green;
backend untouched.

**Plan 67 slice A — library-fuzzy adoption + palette motion (2026-09-09,
user-approved; ADR-148):** as-built discovery: a `CommandPalette` (Ctrl/Cmd+K,
AppShell) and a local `lib/fuzzy.ts` scorer shipped in the init commit — the
plan's greenfield assumption was wrong, so slice A landed as adoption + polish:
the duplicate local scorer is deleted in favor of the library
**`@neuronection/assistant-ui/fuzzy` `searchScore`** (nine consumer files keep
the `fuzzyFilter` glue API; matches now gain bounded typo tolerance —
MaterialPicker filter test re-pointed from exclusion to ranking assertions),
and the palette is animated via new **`lib/motion.ts`** presets (AnimatePresence
backdrop/panel spring, staggered rows, `layoutId` active-row pill) with a
reduced-motion contract test (stagger collapses, transitions go instant).
Frontend 1,099 green (+2), lint/typecheck/build green; backend untouched.

**Plan 67 slice B — quick capture (2026-09-09):** the capture sheet
(`components/capture/QuickCapture.tsx`) slides up via the motion presets —
`Ctrl/Cmd+Shift+K` or the palette's new first action opens it, `Ctrl/Cmd+Enter`
saves a markdown note into the **scratchpad course** (title derived from the
first content line, ≤80 chars + ellipsis, tagged `quick-capture`), and closing
springs away into the library **`UndoNotice`** shim (new `components/ui/undo-
notice.ts`) with Undo (deletes the note) and Open (navigates) — empty captures
show an inline failure hint and never hit the API. Home gains a recent-captures
strip (last 3 scratchpad notes as chips + a capture button). `useCaptureStore`
(zustand) owns open state; `capture.*` i18n keys added. Frontend 1,101 green
(+4), lint/typecheck/build green; backend untouched.

**Plan 67 slice D — material tags & ⭐ favorites (2026-09-09; ADR-149):**
migration **0057** adds `materials.tags` JSON + `materials.starred` boolean
(+ partial index where starred); both ride `PATCH /materials/{id}` via new
`MaterialsService.set_tags`/`set_starred` (tags normalized by the shared
`normalize_tags`), the list endpoint filters `tag=`/`starred=`, and both
fields round-trip in `ca-course/v2` bundles (export + import + round-trip
test). UI: the material header meta row gains notes-style tag chips
(add/remove inline) and a ⭐ toggle in the primary row; the Library gains a
starred-only filter toggle, tag filter chips, and Star/Unstar in the
material context menu (single + multi-select). OpenAPI regenerated.
Head-assertion tests bumped to `0057_material_tags_starred`. Backend 1,062
green (+3), frontend 1,101 green (api-schema drift clean), lint/typecheck/
build green.

**Plan 67 slice E — URL→material importer (2026-09-09; ADR-150):** new
`POST /materials/import-url` reuses the chat FETCH pipeline (`perform_fetch` →
html→markdown, no provider key needed) and lands pages as text materials
through the standard `create_text` + ingest path, with a provenance header
block + `provenance {source: web, url}`, optional node assignment
(`rationale: web import`), an honest 422 on unreachable/non-200/empty pages,
and a per-profile in-flight guard (409). Frontend: `UrlImportDialog`
(URL + course picker, inline error, navigates to the imported material on
success) mounted in AppShell, opened from the Library "New…" menu and a
palette action. Backend 1,065 green (+3), frontend 1,104 green (+3),
lint/typecheck/build green.

**Plan 67 slice C — snap a screen region into notes (2026-09-09; completes
plan 67):** the plan-40C crop engine moved out of the chat composer into
`components/capture/RegionCrop.tsx` (pure + controlled — open/labels in,
`onCapture(png, rect)` out; chat's `ScreenshotDialog` re-composes it and the
chat composer suite passes unchanged), and notes gain two entry points: a 📸
toolbar button in the note editor (optional `onSnapRegion` prop threaded
through `MarkdownEditor` → `StudyToolbarButtons`, beside the pen) and a
"Snap a region into note" palette action. One flow
(`components/capture/SnapIntoNote.tsx`, AppShell-mounted, `useCaptureStore`
snap slice + note-target registration): RegionCrop → thumbnail preview
popover (motion presets, reduced-motion instant) with Insert/Cancel — no
silent inserts. Insert creates a **strokeless drawing**
(`POST /notes/{id}/drawings` with `strokes: []` — notes `DrawingIn` dropped
`min_length`; OCR enqueue is skipped for strokeless drawings on both the
create and update paths, so no `drawing_ocr` job ever spawns) and drops the
`ca-drawing://N` ref at the cursor (new `MarkdownEditorApi.removeDrawing`
backs Undo, which deletes the drawing — the server strips the body block, the
editor strips the node); with no note editor open the shot lands in a new
note in the current course, else the scratchpad. Shared crop strings moved
from `chat.composer.*` to neutral `capture.*` keys (values unchanged).
OpenAPI regenerated — en route this surfaced that slice E's `import-url`
endpoint had never been exported (the committed schema was stale; the CI
drift guard would have flagged it) and that the current FastAPI version now
qualifies the colliding `DrawingIn` schema names. Backend 1,066 green (+1:
strokeless drawing → 201, no `drawing_ocr` job, `ca-drawing` body-block
round-trip, delete), ruff/mypy clean; frontend 1,118 green (+14),
lint/typecheck/build green.

**Next round pointer:** **plan 67 — capture & library organization COMPLETE
2026-09-09** (all five slices A–E landed — see the entries above). Next, same
approval: **plan 68 — reminders, calendar & review nudges** (notification
center, planner week view, ICS export, due badges; ADRs 151–153), then
**plan 69 — languages & motion polish** (en/Ελληνικά/Deutsch switcher +
completion pass, interaction sweep; ADRs 154–155). **Plan 71 — library
placement references** (per-item reference-count badges + right-click
"Linked locations" details over the existing course-tree placement data;
ADR-159) is user-approved 2026-09-09 as a follow-up round — plan doc under
`dev/plans/71-library-placement-references.md` (local-only; numbered 71
because plan 70 — compose quality & scale — exists in the family planning
record). Plan 66 (chat as
a working actor) COMPLETE 2026-09-09 — see its entry above. Plan docs are
local-only under `dev/plans/`.

**Plan 62 slice G follow-up — editor fills its container, content scrolls
inside (2026-09-07, supersedes the sticky-toolbar attempt):** the shared
`MarkdownEditor` is now a flex column that **fills the height its container
gives it** — chrome wrapper + library card (`data-as=rich-text-editor`) run
`flex flex-col flex-1 min-h-0` via host-side arbitrary selectors (no library
change), and the content wrapper (`EditorContent`'s div, targeted as the
card's non-toolbar child) is `flex-1 min-h-0 overflow-y-auto` — so the toolbar
is a permanently visible sibling and only the text scrolls. Height chains
wired through the split panes (`flex h-full min-h-0 flex-col`, pane no longer
scrolls — the StudyPaneHeader now stays visible too), the material view body,
the extraction QA editor and the note editor; in containers without a definite
height (note drawer) `flex-1` degrades to the previous grow-and-scroll
behavior. Frontend 997 green, lint/typecheck/build green; backend
untouched-green (1,033).

**Plan 62 slice G follow-up — sticky editor toolbar (2026-09-07):** with the
height cap gone, the editor toolbar now pins (`sticky top-0`, surface
background) to the top of its scrolling container while a long document
scrolls, so formatting controls stay reachable at any depth; it still
reveal-on-hover/focus. In the note drawer the toolbar slides beneath the
drawer's own sticky header (lower paint order) — recorded as accepted behavior.
Frontend 997 green; backend untouched-green (1,033).

**Plan 62 slice G — editors use the full height (2026-09-07, user-reported):**
the shared `MarkdownEditor` content area dropped its `max-h-[65vh]` +
internal-scroll cap — the editor now grows with its content and the surrounding
scroll container takes over (AppShell main, split panes, drawer panel and the
text-file dialog body are all `overflow-y-auto`), so the extraction/formatted
QA editor and the note editor fill the pane instead of floating in a 65 vh box.
Tests re-pointed (recomposition asserts `min-h-56`; the pinned-toolbar case
became "body grows with content, toolbar above it") — frontend 997 green,
lint/typecheck/build green; backend untouched-green (1,033).

**Plan 62 slice F — one ⋯ menu for all material verbs (2026-09-07, user-requested
follow-up):** **Print, Save as material and History move out of the extraction
toolbar's ⋯ into the material header's ⋯ "More material actions"** — the header
menu now holds Export .md / Print / Save as material / History (mindmaps keep
the old gating: no History item), and the extraction toolbar slims to
genuinely content-scoped controls only (version meta + ▶ read-aloud + ✎ edit).
The whole ⋯ is **hidden while no extraction exists** (matching Ask AI's gating —
replacing the disabled-Export item). Mechanically the print overlay,
ExtractionHistoryDialog state and the derive mutation + feedback card hoisted
from `ExtractionView` into `MaterialDetailBody` (one instance per material —
correct for the side-by-side twin-view case); derive keeps its
materials/tree/node-workspace invalidations and the five derive tests moved
with it. ExtractionView's overflow tests re-pointed; MaterialDetailBody gains
derive/print/history/mindmap-gating coverage (+3 net) — frontend 997 green,
lint/typecheck/build green; backend untouched-green (1,033).

**Plan 62 slice E — header declutter: views + description + export demoted
(2026-09-07, user-requested addendum):** every material now switches views via
the compact **Switch view dropdown** (new `ViewMenu`, the family `Menu`
compound with `MenuCheckboxItem` checks, trigger showing the active view) —
text files offer Formatted/Raw text, PDFs/scans Extraction/Original/Side-by-
side; the tab strip (and the `role=tab` grammar) is gone from the reading
header entirely (the legacy `?tab=description` deep link resolves to
formatted). **Description** is no longer a view at all: it renders in an **ⓘ
"Material description" popover** (library `InfoButton`) in the header meta row,
shown only when a description exists. **Export .md** leaves the primary row for
the new **⋯ More material actions** overflow (still disabled without an
extraction, `aria-disabled`). `tab_description`/`descriptionEmpty` keys
retired. Frontend 994 tests green, lint/typecheck/build green; backend
untouched-green (1,033).

**Plan 62 COMPLETE — split-study pane restructure (2026-09-07, user-approved;
slices A→D in four commits, zero backend, ADR-139):** recap of the round —
**A (per-pane close):** the note editor's ✕ closes **only the notes side**
(`NodeWorkspace.closeNotes` drops `study`, keeps `material` — the material
drawer takes its place) while pane ✕/Esc still exits everything;
`NoteEditor.onRequestClose` composes an autosave `saveNow()` flush, delete
routes through it without the flush, and `closeLabel` threads through
`LazyNoteEditor`/`FocusShell` so the note-side ✕ announces "Close notes".
**B (one header per pane):** the "Study — {title}" band retired
(`study.paneTitle` gone); material header shows the title once with meta chips
+ tab strip merged into the band; the inline note header hosts the inline
title, tag chips + Add-tag (dangling "Add tag_" line gone) and an autosave chip
that is a **Save now** button while dirty. **C (chrome):** shared
`MarkdownEditor` toolbar quiet-by-default (hover/focus-within reveal) on every
surface, math-hint paragraph → `InfoButton` "?" popover, divider visible
(grip dots) + arrow-key resize + double-click 50/50 reset. **D (empty
state):** fresh empty notes show a dashed quick-start strip (quote bridge /
math+mermaid syntax / honest AI note) dismissed on first content. Deviations
recorded in the plan: drawer-mode NoteEditor keeps the FocusShell header
(breadcrumbs, Save button, primary Study-alongside); tiptap CSS placeholder
dropped in favor of the strip. Frontend 994 tests green (+16 over the round),
lint/typecheck/build green; backend untouched-green (1,033).

**Plan 62 slice D — new-note empty state (2026-09-07):** the inline (split)
`NoteEditor` renders a muted dashed quick-start strip above the editor when the
note body is empty and there are no drawings — `notes.emptyState.quote`
("Select a passage … Quote into note"), `.math` ("$…$ inline, $$…$$ display,
```mermaid diagrams") and an honest `.ai` line ("✨ AI actions become useful
once the note has content") — dismissed the moment the note has content
(`data-testid=note-empty-state`); the standalone drawer never shows it (the
quote bridge is split-only). Tests +2 — frontend 994 green,
lint/typecheck/build green; backend untouched-green (1,033).

**Plan 62 slice C — editor chrome + divider affordances (2026-09-07):** the
shared `MarkdownEditor` toolbar (all surfaces: notes, extraction QA, text-file
dialog) is now quiet-by-default — wrapped in a `group/editor` chrome div whose
`[role=toolbar]` fades to `opacity-0` and reveals on hover/focus-within
(150 ms, `motion-reduce`-safe; buttons stay in the tab order, so keyboard focus
keeps the toolbar visible). The permanent `editor.mathHint` paragraph under
every editor is replaced by a library `InfoButton` (`?`, "Editor help") at the
toolbar's end whose popover carries the same hint under the new
`editor.mathHelpTitle` title — one row of chrome gone. The split divider gains
a visible grip (three dots, hover/focus highlight), `aria-valuenow/min/max`,
ArrowLeft/ArrowRight keyboard resize (±5, clamped 30–70, persisted), and
double-click reset to 50/50 that clears the per-course `ca-study-split:{course}`
key (`study.resetSplit`). Tests: recomposition popover/gone-paragraph case +
divider keyboard/reset case (+2 net) — frontend 992 green, lint/typecheck/build
green.

**Plan 62 slice B — one `StudyPaneHeader` per study pane (2026-09-07):** the
split-study panes (and the library material page/drawer) now share one header
anatomy — new `components/layout/StudyPaneHeader.tsx` (title / meta chips /
tabs / primary verbs / overflow / close slots, one wrapping band, shared
baseline). **Material side:** the pane-level "Study — {title}" band is deleted
(`study.paneTitle` retired); `MaterialDetailBody` renders the shared header
with the material title shown once, the status/course/assigned chips as meta,
the Formatted/Raw/Description (or Extraction/Original/Side-by-side) tab strip
merged into the band (segmented-control styling, same `role=tab` grammar), and
Ask AI / Take notes / Export .md / reading-status dropdown as primary; the
pane's close ✕ rides this header (`onClose`/`closeLabel` props). **Note side
(inline split mode):** `NoteEditor` renders the same header — inline-editable
title, tag chips + Add-tag moved out of the content flow (kills the dangling
"Add tag_" line), an autosave status chip that is a **Save now** button while
dirty (replaces the standalone Save in the split; the drawer keeps its full
action row), ReadAloud/AI/flashcards as primary and Print/Export/History/Delete
in the ⋯ menu (`notes.saveNow` key added). Drawer-mode NoteEditor (note drawer,
focus page, print) keeps the FocusShell header with breadcrumbs — recorded as a
plan deviation. Tests: +5 net (header band anatomy, pane-close threading,
no-pane-title-band, inline header hosting, drawer-only study-alongside
re-pointed) — frontend 990 green, lint/typecheck/build green.

**Plan 62 slice A — split-study per-pane close (2026-09-07, ADR-139):** the ✕ on
the note editor's header now closes **only the notes side** of the split-study
view — `NodeWorkspace.closeNotes` drops just `study` from the URL, keeping
`material`, so the material drawer takes the right pane's place (previously the
single pane ✕ exited everything; the user ask that spawned plan 62). The note
editor gains `onRequestClose` (composed with an autosave `saveNow()` flush
before close; delete routes through it too, without the flush) and a
`closeLabel` threaded through `LazyNoteEditor`/`FocusShell` so the note-side ✕
is announced "Close notes" vs the pane-level "Close"; Esc still closes the whole
pane. Tests: SplitStudyPane probe (+2), NoteEditor per-pane close (+4 incl.
dirty-flush and inline delete), NodeWorkspace URL contract (+1) — frontend 985
green, lint/typecheck/build green; backend untouched-green (1,033).

**Reading-view header cleanup (2026-09-07, user-requested UI modernization):**
the material page's top action row now holds the frequent verbs together —
**Ask AI moved up from the extraction toolbar** to sit beside **Take notes**
and **Export .md** (rendered only while an extraction exists, matching the old
gating) — and the **unread/reading/studied** pill trio became the new
`StudyStateMenu` **compact dropdown** (outline button: colored status dot —
muted/amber/green — + current label + chevron; `MenuCheckboxItem` menu with a
check on the active state; safe status narrowing from the API's `string`).
The extraction toolbar slims to **read-aloud ▶ + Edit pencil (icon button,
same accessible name) + a ⋯ `PopoverMenu` More-actions menu** holding
**Print, Save as material, History** (mindmap variant: Print + Save as
material; derive rides the menu item's `pending` spinner), and the plain
reading view **regained its Print action** — the `MarkdownPrintDoc` overlay
state existed but only the mindmap branch rendered it. Library adoption: the
`popover-menu` shim now also re-exports the family menu compound
(`Menu`/`MenuTrigger`/`MenuContent`/`MenuItem`/`MenuCheckboxItem`/…).
Tests re-pointed: ExtractionView derive tests open the ⋯ menu first, the
plan-61-B Ask AI test moved to `MaterialDetailBody.test.tsx` (+ new
no-extraction gating and dropdown-switch cases), LibraryPage's study-state
test drives the dropdown. Frontend 978 tests green (+3 net), lint/typecheck/
build green; backend untouched.

**Multi-material Ask AI from the library context menu (2026-09-07,
user-requested plan-61 follow-up):** select one or more materials in the
Library (click / Ctrl-click / marquee) → right-click → **Ask AI about N
materials…** opens a dialog with the selection as attachment chips (kind
icons, titles), the five predefined question chips (multi-aware phrasings —
`askChips` gained `textKeyMulti`) and free text (Enter sends). One chat
session per selection tuple (`chat-store.selectionAsks`, order-insensitive
sorted-id key — re-asking the same set reuses it; new `askMaterials()`
alongside `askMaterial`), course-scoped to the selection, all materials
attached as `material` attachments feeding the READ tool (backend cap 10 —
the dialog warns and disables past it); the quiz-me chip PATCHes quiz-me mode
first, same as the single-material popover. Browser-locked by e2e `05-ask`
S8 (multi-select → menu → dialog → thinking card → streamed reply) plus
dialog/util/store vitest suites. **Same-commit fix — first-run wizard gate vs
scratchpad:** AppShell boots `GET /courses/scratchpad`, which lazily seeds the
hidden `origin=scratch` course; `/onboarding/state`'s `has_course`/`has_material`
counted it, so on fast boots the first-run wizard never opened on a fresh
install (the e2e 01-boot flake was this race, not a test problem). The state
now excludes hidden courses (join-filtered for materials) — regression-tested;
01-boot also gained a reload-retry guard against genuine boot-race fetch
errors. Frontend 975 tests / 8 e2e specs green, backend 1,033 green (verified
against the local library tarball — see the mid-flight note below).

**Mid-flight (other session, 2026-09-07):** study HEAD consumes
`@neuronection/assistant-ui/chat-trace-timeline` (+ chat-tools-catalog /
chat-export), which is pushed in the library but **not yet released** (npm
latest 0.27.0 lacks it; a 0.28.0 changeset is staged). Until 0.28.0 ships,
`pnpm build` against published packages fails on the missing export — local
verification must use the library tarball (`verify-in-app.mjs`).

**Full-page chat fixes — streaming + layout + theming (2026-09-07, live-testing
follow-up to plans 60/61):** the first send on the full-page tutor was dead —
no thinking card, no streaming, reply invisible until a manual refetch. Root
cause chain: `ChatPanel.submit` creates a session then navigates `/chat` →
`/chat/$chatId`; the two routes each mounted their own `ChatPage` +
`StudyChatProvider`, so the remount orphaned the in-flight turn (the new
provider's idle reducer dropped every WS event) and nothing refetched. Fixes:
**router** — `/chat` is now a layout route with index + `$chatId` children
(ChatPage mounts once; navigation keeps the surface alive);
**`useChatSession`** — bridges a just-created session (chat-store ref match)
while the sessions refetch is in flight so the provider prop never flips null
mid-send; **AppShell** — the `/chat`-closes-sidepanel effect is guarded on
`chatOpen` (it wiped the store pin via `setOpen(false)`'s `session: null`,
killing the bridge mid-flight). Regression locks: `ChatPage.streaming.test.tsx`
(vitest, fails on the old router shape) + e2e `02-chat` S5; S4's stale
`/Calculate/` assertion (pre-existing red since the plan-12 card swap — the
family card's accessible name is argument+name) re-pointed to `/CALC/`.
**Same-day follow-up — ask-AI first send invisible:** the reading-view Ask AI
popover queues `pendingSend` *before* the sidepanel mounts; the panel consumed
it in its mount effect and `chat.send` flipped the stream to pending — but the
provider's mount-time reset effect (parent effects run last) saw no
`adoptingRef` match (the ask creates the session outside the panel) and wiped
the turn to idle, so every WS event hit the reducer's idle guard: no thinking
card, no stream, no terminal refetch — the reply appeared only after the next
manual send. Fix: the pendingSend send path adopts the session first
(`chat.adoptSession({ id })`, signature loosened to `{ id: number }`), which
guards the mount reset; regression-locked by a mount-consume streaming test
(fails without the fix). Hardened after a second live report: the e2e mock
provider now paces SSE chunks (~60 ms/token, real-LLM turn length) and two
permanent specs lock the ask flow in the browser — `05-ask` S6 (fresh ask:
thinking card + user message + streamed reply all visible live) and S7 (ask
with the sidepanel already open on another session) — plus a defensive
catch-up guard: the pendingSend send path waits until the provider's session
id matches the store's pinned ask session before adopting+sending, so a
scheduler-deferred store flip can never post to the wrong session.
Dev-server ergonomics: Vite never watches `node_modules` (a negation-glob
un-ignore explodes inotify — ENOSPC in practice), so `frontend/vite.config.ts`
gained the serve-only `assistant-ui-hotswap` plugin — two tiny `fs.watch`
handles on the package scope dir (symlink retargets on version bumps) and the
realpath'd `dist/` (in-place swaps) trigger a debounced dev-server restart;
verified live (file swap → logged restart → fresh module served), gated off
under vitest where the handles held the test server open.
Styling: library `ChatPanel` page variant is now
full-bleed (header/banner/composer span the page, transcript+composer body
centered at `max-w-3xl`) and `ChatMessage` assistant bubbles are
muted/borderless — released as **assistant-ui 0.26.0** (changesets CI;
verified in-app via the tarball flow before release, career suite green too);
study's dep bumped to `^0.26.0`, and the new `frontend/src/theme.css` bridges
the app palette onto `--as-*` incl. a `.dark` remap (library components now
follow dark mode; CSS import order per the library theming guide: styles.css
→ index.css → theme.css). Frontend 963 tests green, lint/typecheck/build
green, `pnpm e2e` 5 specs green; backend untouched.

**Family AI-alignment migration (ADR-0008, family plan 10) — Phase 5 landed
2026-09-04 (branch `ai-align/phase-5`):** the chat turn now also runs as a
checkpointed LangGraph graph (`app/ai/graphs/`) behind `SA_CHAT_ENGINE`
(default `legacy`, zero behavior change); dialect-picked checkpointer + boot
prune; WS event contract byte-identical (parity-tested against the legacy
engine); `scripts/check-ai-alignment.sh --self` strict-clean.

**Plan 12 wave 2 — study swaps: tool cards + turn status on the library
modules (2026-09-06, family plan 12 Phase 4 / ADR-0009):** the last
drift-prone chat copies are gone — `features/chat/ToolCallCard.tsx`,
`TurnTraceStatus.tsx` and the `ChatPanel` `ThinkingDots` are deleted in
the same commit that adopts the library `ChatToolCard` (new
`renderResult` slot keeps every per-tool view app-side:
`tools/registry.tsx` gains `toolCardProps()` feeding icon, summary,
labels, `defaultOpen` for quiz cards and the result-view slot) and
`ChatTurnStatus` (`variant="row"` replaces `TurnTraceStatus` with the
same 100 ms tabular-nums timer; `variant="card"` replaces
`ThinkingDots`). `.chat-dot` CSS removed (library ships
reduced-motion-safe dots); `chat.tool.toggle` key retired. Tests
re-pointed before delete: quiz grading now drives the library card via
the registry adapter (`tools/toolCardQuiz.test.tsx`), streaming/panel
assertions use the family card's accessible names, registry tests cover
the prop mapping. `TraceTimeline`, `ContextPanel`, `AttachMenu`,
`ComposerExtras`, `ToolsDialog`, export util and the WS client stay
app-side per plan 12 §2 rulings.

**Plan 60 — chat surface adoption on the library core COMPLETE
(2026-09-06, family plan 11 Phase B / ADR-0009):** the sidebar and full-page
tutor chat are rebuilt on `@neuronection/assistant-ui@0.24.0` chat modules —
`ChatPanel` (page/sidebar hosts), `ChatTranscript`, `ChatMessage` +
`MessageVariantSwitcher`, `MarkdownSurface`, `ChatReasoning`, `ChatComposer`
(IME-safe), `ChatBranchTree`, `ChatSessionList`, `PopoverButton` — with the
WS→family-event `chatTransport.ts` adapter (run-id gated, edit/regenerate
intents) and the one-live-turn-per-surface `StudyChatProvider`. All parity
features survived: blocks/widgets via the content rule, mentions via the
markdown `a` override + tolerant url transform, variants/branching, thinking
stream (same `ca-chat-reasoning-open` key), tool cards, trace timeline,
attach/equation/draw/screenshot, dictation, export .md, 90 s watchdog,
error banner. Same-commit deletes: `StreamingBubble`, `ReasoningBubble`,
`useStreamBuffer`, `BranchTreePanel` (→ `BranchTreeButton`), the old
`ChatSessionList` internals and the 1,364-line shell/composer/live state
machine (~2,000 local lines gone). Frontend 925 tests green (was 906),
lint/typecheck/build green, `pnpm e2e` green against the mock provider.

**Plan 61 — reading-view "Ask AI" + sidepanel coexistence COMPLETE (2026-09-07,
ADR-137/138; `dev/plans/61-reading-view-ask-ai-and-chat-coexistence.md`, mirrored
to the family dev repo).** Zero-backend frontend round, slices A→D in one
landing: **A (ask bridge, ADR-137)** — `chat-store` gains a `pendingSend`
channel (`{content, attachments, mode: prefill|send}`) consumed by the
sidepanel ChatPanel after session adoption (one send path — callers can never
race the run-id-gated WS transport), per-material session pins
(`viewerAsks[materialId]`, lazily created titled after the material, scoped to
its node), `useMaterialAsk()` as the single entry (grounding via message
attachments feeding the mention registry/READ — never pasted ids; selections
embed as 1,500-char-capped blockquotes; optional `quizMe` PATCHes the session
before queueing), and the chat width lifted into the store (localStorage
persistence unchanged). **A-fix (2026-09-07, live-testing follow-up):** the
pendingSend send path now invalidates `chat-messages` + `chat-sessions` right
after the POST — without it a popover-sent question stayed invisible until
the next manual send (submit's invalidation was the only refetch trigger);
pinned by an extended ChatPanel test. (Related live-testing console warning —
`ChatDrawer` forwardRef — is an `assistant-ui@0.25.0` build artifact, already
fixed + changeset'd in the library checkout awaiting release; cosmetic.) **B (Ask AI popover)** — `AskAiPopover` in the reading
header (attached-material chip + typed `ASK_CHIPS` set + free text; chips fill
the input, **one explicit Ask click sends** — the quiz-me chip retains its
flag only while its text is unedited). **C (yield rule, ADR-138)** — FocusShell
overlay panel *and backdrop* inset by the live chat width (un-maximized clamps
`max(16rem, min(760px, 100vw−2rem−inset))`, maximized yields to
`100vw−inset` without flipping persisted state; backdrop stays clickable-outside),
200 ms `motion-reduce`-safe entrance/resize animations on the chat panel.
**D (selection-anchored ask)** — capture extracted into shared
`lib/useTextSelection.ts`; the floating `SelectionToolbar` (Ask always,
quote-into-note where a note target exists) lives in `ExtractionView`
(`onQuoteSelection` threaded through `MaterialDetailBody`; `SplitStudyPane`
dropped its private capture — quote behavior preserved, its suite re-pointed
to the wiring probe). Frontend 961 tests green (+36), lint/typecheck/build
green; backend suite untouched-green (1,032).

**Next up:** **plan 59 — assistant-ui editor & AI-surface promotion —
study-side COMPLETE (2026-09-05, ADR-133)**: library modules
`rich-text-editor` / `dictation` / `ai-text-transform` / `text-diff-view`
(0.20.0) adopted study-side with shims; the library `rich-text-editor`
gained the host API (full-override `extensions`, `contentClassName`,
`onReady`, `headingLevels`, can-undo/redo history buttons + the
`setEditable` mount-emit fix) released as **0.23.0** (0.22.0 carried the
API, 0.23.0 the fix), and the shared `MarkdownEditor` is **recomposed onto
the primitive** — zero-behavior (all editor suites pass unchanged), editor
still a lazy chunk; `useEditorTransform`, local dictation/diff state
machines and the local `useEditor`/toolbar are gone. Plan 59's only remainder
is career-side (Slice F). **In progress: plan 52 (topic explorer & course
genesis, ADR-134)** — Slice A (Scratchpad) landed 2026-09-06: a per-profile
hidden `origin=scratch` course (0053 `courses.origin`/`hidden`) seeded lazily,
hosting notes/files/chat uploads with **Promote-to-course** (subtree move to a
new course; concept coverage honestly dropped; scratch is excluded from exam
forecasts and profile-wide recommendations). **Slices B + C landed 2026-09-06
(ADR-135):** course genesis — Home "Generate a course from a topic" → 3-step
dialog (topic+level → editable outline → options) over `POST
/courses/genesis/draft` (outline task + deterministic validators) and `POST
/courses/genesis` (origin=genesis course, `ai_hint` objectives, budget cap
`genesis_task_cap` default 60, one cancellable `genesis` job with per-task
failure isolation generating `lesson` compose + 5-question quiz + 10
flashcards per chapter); the `lesson` compose kind (model-knowledge
expository lessons, one-live-artifact per node) is slice C. **Slice D landed
2026-09-06 (ADR-136):** SEARCH/FETCH chat research tools over a configurable
web search provider (`app/search/provider.py` — Tavily-compatible POST or
SearXNG JSON adapter, flavor+base_url in profile preferences, API key in the
keyring, Settings → Providers → Web search card), budgets 2/1 per turn,
results never persisted, advisory `sources_cited_when_search_used` contract,
citation-chip tool cards, and optional web-grounded genesis outlines
("Drafted with sources:" footer). **Plan 52 COMPLETE. In progress: plan 53
(planner & expression, ADR-118)** — Slice A (study planner) landed 2026-09-06:
`plan_items` (0054, placement-composite-FK so items follow node merge-delete
and promote-to-course), deterministic exam-paced plan drafting
(`POST /courses/{id}/plan/generate` — untouched nodes paced across the days
left, weak-concept practice items from the diagnostics, weekly review
milestones, all as keep/discard drafts; re-generate respects done/manual),
root-only **Planner** workspace tab (check-off, drag-between-days, +1-day,
manual items) and a **This week's plan** strip on Home. **Slice B landed
2026-09-06:** mastery-weighted **exam-readiness forecast** — `exam_status`
gains readiness 0–100 (50 % coverage + 35 % in-scope answer accuracy +
15 % recent trend, `course_readiness` in metrics.py), improving/steady/
declining trajectory, up-to-3 weakest concepts as evidence, honest
not-enough-data gate (<3 answers); ring + trajectory on the Home exam card,
chip on the Planner tab, and generated plans schedule practice first when a
forecast exists. **Slice C landed 2026-09-06 (ADR-121):** **Quiz-me chat
mode** — per-session `chat_sessions.quizme` toggle (0055 + `quiz_pending`
JSON) swaps in the assessing system prompt and the `QUIZ` tool (native
schemas; 2/turn); the server validates the question, holds the expected
answer in `quiz_pending`, renders an interactive card, and grades answers
deterministically via `POST /chat/sessions/{id}/quiz-answer` (choice match /
equivalence chain / normalized text + accept variants) — the expected answer
never enters any model-visible payload; verdict + expected answer render on
the card. **Plan 53 round 2 (2026-09-06) — E + F + leftovers COMPLETE:** the
C-gap decision (verdicts count in daily telemetry via 0056 `quizme_answers` —
no synthetic Answer row), D2 (Teach-back joined the workspace entity menu via
`courseNodeSource`), **slice E (read-aloud TTS, ADR-119)** — `speech`
capability + `tts` task + `LLMGateway.speak` (openai_compatible `/audio/speech`
mp3; Gemini TTS generateContent with server-side L16→WAV wrapping; anthropic
unsupported) behind `POST /ai/speak` (≤10k chars, ephemeral, ledgered
`[tts N chars]`, 409/429/502) and the frontend `useReadAloud` engine picker —
browser `speechSynthesis` zero-config fallback gated on real voice
availability (`getVoices()` after `voiceschanged` settle, one-shot probe —
voiceless WebKitGTK hides ▶ buttons) with the provider task as the quality
path; ▶ buttons on extraction reading view, note editor, chat assistant
messages, and ai-composed lessons; playback stop + speed cycling; a
deterministic LaTeX strip-and-speak helper (`frac`→"over", `\sin`→"sine",
sqrt→"square root of", command/brace stripping) with client-side sentence
chunking under the 10k cap — and **slice F (print/PDF engine, ADR-120)** —
`components/print/` kit (`PrintDoc` cover/context shell, print stylesheet with
`body *` visibility flip, page-break/answer-line/cut-card classes,
`useAutoPrint`), quiz **exam paper + answer key** via `?print=paper|key`
(replacing the navigate-and-print hack; server-authoritative
`GET /quiz/activities/{id}/answer-key` renders every question kind's key
deterministically), **flashcard cut-out sheets** over the due set (fold-line
grid) on the Cards segment, a **Print** action on the extraction reading view
(`MarkdownPrintDoc` over `MarkdownSurface`), and the **Planner week sheet**
(Mon–Sun day groups). **Plan 53 COMPLETE.**

**Plan 51 (AI-native
answer types) COMPLETE (2026-09-03)** — A number-line, C error-spotting,
F table-fill, E composite follow-through, G graph-reading, D Pyodide code
execution, H item-level Elo all landed (ADR-112…125; image-region
hotspot/label hotspots deferred until an image-authoring surface exists, see
ADR-124 note). **Plan 50 COMPLETE (2026-09-02)** and plan 48 landed
2026-09-01 — see the 2026-09-02 changelog entries.
...
| Frontend skeleton (Vite, Tailwind, shadcn, i18n harness) | done | React 19 + TS strict, Tailwind 4 tokens, shadcn (button/card + components.json), react-i18next with **en/el/de catalogs shipped** (plan 69: locale switcher in Settings → General, locale-aware `lib/format.ts`, `lib/locales.ts` completeness gate; `no-literal-string` lint rule), TanStack Router/Query, Zustand, framer-motion (`lib/motion.ts` = single motion-preset source per ADR-155). **Bundle split (plan 17 G)**: note editor behind `React.lazy` (tiptap in a lazy chunk); vendor chunks `react-vendor`/`framer-motion`/`katex` via rolldown `codeSplitting.groups`; entry 697 kB min / 192 kB gzip (was 2,067/627) |
...
- 2026-09-09 — **fix(ai): graph chat engine no longer leaks a round's final token
  ...
  merged `updates`/`messages` queue — so under CI load the round's last chunk
  (`*21\n`) was delivered *after* the CALC `tool_call`. Once the round boundary
  (`on_round_stream_end`) had flushed the pending tool line, the straggler
  completed a non-tool line and streamed as answer text, breaking
  `test_graph_tool_round_matches_legacy`'s engine-parity assertion (and would
  render a stray fragment in the tutor's live answer). `_DeltaPump` now closes
  at the round's stream end (dropping any further text) and the adapter reopens
  it when a chunk from a later `langgraph_step` arrives (next round / node
  execution; step presence verified against the pinned langgraph). Drops are
  fail-open: without attributable step metadata nothing is discarded. Unit test
  `test_round_close_drops_straggler_delta` pins close → straggler-drop →
  reopen; graph-turn file green 3× under `pytest -n 4`; full backend suite
  1,067 passed.
- 2026-09-09 — **fix(ui): route transitions no longer remount the outlet subtree;
  full-page tutor turn survives session-creation navigation.** The plan-69
  `RouteFade` wrapped the AppShell outlet in `AnimatePresence mode="wait"`
  keyed by the full pathname, so `/chat` → `/chat/<id>` (the navigation
  `onSessionCreated` fires mid-turn) unmounted and remounted the whole chat
  subtree — the fresh `StudyChatProvider` reset the in-flight turn stream
  (thinking status gone; the WS topic resubscribed after the stream went
  idle, so every turn event was dropped), polling stopped, and the finished
  reply never rendered. CI's release e2e smoke caught it (S5). `RouteFade`
  now keys by the top-level area (`/chat`, `/courses`, …) and plays the
  route enter preset without `AnimatePresence` (its child caching re-mounts
  TanStack outlet content on any navigation even under a stable key — found
  by bisecting against a plain `<Outlet/>`), so within-area navigation keeps
  the subtree and its state mounted; cross-area swaps still fade/slide in
  (reduced-motion safe via the preset's instant transition; first app mount
  skips the animation, preserving the old `initial={false}`). Regression
  tests: within-area navigation does not remount the layout, cross-area
  swaps replace content. Full gate green (backend 1,066, frontend 1,153
  tests, build, e2e 8/8).
- 2026-09-09 — **feat(i18n+ux): the app is fully usable in Greek and German + motion
  sweep (plan 69, ADR-154/155).** Settings gains a **General** tab (first) with a
  language picker — a searchable dropdown on the library `Combobox` (native names,
  fuzzy search, instant apply via `i18next.changeLanguage`, persisted per device in
  localStorage `ca-locale` (`lib/constants.ts`), html `lang` kept in sync) that also
  rides the **first-run wizard's welcome step** (`components/LanguagePicker.tsx` is
  the shared picker) and **General is the default Settings tab** (`/settings` opens
  it; `?tab=` deep links unchanged); the picker lists only locales
  passing a runtime completeness gate (`lib/locales.ts`, ≤2% missing vs the `en`
  source; CI's `pnpm i18n` still demands 100% — el/de are at 1,959/1,959 keys after
  the family translation pass `--fix --apply` + `--review --apply` over
  `scripts/translations/`, with math glossaries in `glossary.{el,de}.local.json`).
  Dates/times/numbers moved to locale-aware `Intl` formatting through the new
  `lib/format.ts` (`formatDate`/`formatDateTime`/`formatWeekdayLong`/`formatMonth`/
  `formatNumber`/`formatPercent`) across planner day headers, due chips (planner,
  Home upcoming, FSRS due-at), exam countdown, history dialogs, jobs, backups,
  trash, scores and task-spend; fixes a latent drift where an OS in a non-English
  locale rendered mixed-shape dates inside the English UI. el/de use the same CLDR
  `_one`/`_other` categories as en, so the strict key audit needed **no** plural
  variance exception (plan 69's anticipated audit-rule change dropped). Motion:
  `lib/motion.ts` is now the single source for every animation archetype (new
  `route`/`collapse`/`step`/`enter` presets; `row` gained an `exit`); the five
  ad-hoc framer configs (proposal payload disclosure, context panel, quiz question
  swap, exercise step swap, Home cards) migrated onto presets; new shared
  components — `RouteFade` (route-level cross-fade in the AppShell outlet,
  120 ms fade/slide-up), `SegmentedControl` (`layoutId` sliding indicator, pill +
  underline variants; adopted in PracticeTab, NodeWorkspace tabs, CourseSettings
  sub-pills, AI-helper review tabs, extraction/new-text-file edit tabs — JobsPage
  filters stay `<Link>`s on purpose), `SuccessBurst` (one-shot SVG check-draw on
  proposal approval) and `Stagger`/`StaggerItem` (list mount stagger + `AnimatePresence`
  removal; EntityItems grid/list, planner items incl. done list, practice cards);
  every tailwind `animate-in` entrance now carries `motion-reduce:animate-none`;
  `test/motion.ts` generalizes the reduced-motion vitest helper
  (`rendersIdenticallyUnderReducedMotion`) and every touched surface asserts it.
  Docs: `docs/usage/language.md`; README bullets. Full gate green (backend 1,066,
  frontend 1,148 tests, build, `pnpm i18n`).
- 2026-09-09 — **chore(deps): @neuronection/assistant-ui ^0.29.0** (published
  today) — picks up the chat-tools-catalog header hardening (scope chip capped
  at 40% with ellipsis + `title` tooltip, name/title slot wraps; the app-side
  mapper already keeps scope sentences out of the chip) and chat-trace-timeline
  observability, retiring the tarball overlay (manifest back on the published
  range, `minimumReleaseAgeExclude` gained `|| 0.29.0`). Frontend gate green:
  lint/typecheck/1097 tests/build.
- 2026-09-09 — **fix(chat): tools-dialog rows cropped — scope sentences no longer
  rendered as header chips.** The backend sends `scope` as a full sentence; the
  library `ChatToolsCatalog` renders `scope` as a shrink-0 uppercase chip sized to
  max-content, so each pill filled the row, pushed the chevron off the card and
  collapsed the `min-w-0` name span (every collapsed row cropped, names invisible).
  App fix: `toolCatalogEntry` (`features/chat/ToolsDialog.tsx`) folds the scope
  sentence into the expanded card's description (fully visible, wrapped) and stops
  passing the chip field; test pins the combined description. Library hardening in
  `../assistant-ui` (patch changeset, rides next release): scope chip capped at 40%
  of the row with ellipsis + native `title` tooltip, name/title slot wraps
  (`break-words`); module tests, LongScopeAndName story and docs page updated there.

- 2026-09-06 — **feat(print+tts): plan 53 round 2 — read-aloud TTS (E),
  print/PDF engine (F), quiz-me daily credit, teach-back menu entry — plan 53
  COMPLETE.** Backend: `Capability.SPEECH` + `tts` task (`app/ai/tasks.py`,
  `speech` cap inferred for TTS-class model ids), `app/ai/speech.py`
  (`speak_with`: openai_compatible `POST /audio/speech` → mp3; google
  `generateContent` `responseModalities:["AUDIO"]` with L16/PCM → WAV header
  wrap; anthropic → `SpeechUnsupported`), `LLMGateway.speak` (retry/fallback
  chain, ephemeral, ledger prompt `[tts N chars]`), `POST /ai/speak`
  (`{text ≤10k, voice?}` → audio bytes + `X-Speech-Model`; 409/429/502);
  migration **0056 `quizme_answers`** (profile/session FKs, correct,
  created_at) recorded on `POST /chat/sessions/{id}/quiz-answer` and folded
  into `daily_history` (answers_n/correct_n beside `Answer` rows — closes the
  53-C accept gap without a synthetic Answer row); `GET
  /quiz/activities/{id}/answer-key` — deterministic per-kind key strings
  (`services/study/answer_key.py`: choice letters, truefalse, multi, text +
  accept, numeric ±tolerance, equation via sympy_check, numberline
  points/intervals, table cells, composite parts, graph point/expression,
  code cases). Frontend: `lib/readAloud.ts` (`stripMathForSpeech` +
  `mathToSpeech` LaTeX→words, `chunkForSpeech` sentence splitting ≤9k,
  `useSpeechAvailable` voice-availability gate — `getVoices()` after
  `voiceschanged` + 1 s settle, so voiceless WebKitGTK hides ▶ honestly,
  `useReadAloud` engine picker provider-first over browser
  `speechSynthesis`, stop + 4-step rate cycling) + `ReadAloudButton` wired
  into the extraction reading view, NoteEditor header, chat assistant
  message meta, and ai-composed lessons via `lib/api` `speakText`;
  `components/print/` (`PrintDoc` cover shell + no-print toolbar slot,
  print.css visibility-flip stylesheet, `useAutoPrint`, `MarkdownPrintDoc`);
  quiz print modes `?print=paper|key` render `QuizPrintPaper` instead of the
  runner (answer lines + lettered options; key from the answer-key endpoint;
  PracticeTab row menu split into *Print paper…* / *Print answer key…*);
  `FlashcardPrintSheet` cut-out grid over the due set (Cards segment, Back +
  Print toolbar); extraction reading view printer icon → `MarkdownPrintDoc`
  overlay; `PlannerWeekSheet` (Mon–Sun groups) behind a *Print week* button.
  D2: **Teach back** joined the workspace entity menu (`EntityActionHandlers.teachBack`
  → `createTeachBack({node_id})` → exercise player). Library bumped to
  **assistant-ui 0.25.0** (chat-turn-status/renderResult now published).
  Tests: backend 1032 (+ answer-key, speak, quizme-credit), frontend 936
  (+ read-aloud, entity-menu teach-back). Docs: features.md (I14/I16),
  ai.md (speech/tts), data-model.md (0056), usage/printing.md (new),
  usage/chat.md (quiz-me credit).
- 2026-09-06 — **feat(quizme): plan 53-C — Quiz-me chat mode with
  server-side deterministic grading (ADR-121, plan 53 `dev/plans/53-…md`).**
  A per-session **Quiz-me** toggle (header button; `chat_sessions.quizme`,
  migration 0055 with `quiz_pending` JSON) swaps in the assessing system
  prompt (one question at a time, escalate, never reveal answers) and teaches
  the `QUIZ` tool (native schema always available; prompt-doc appended only
  in quiz-me; 2/turn budget). The model's `QUIZ` call carries question +
  expected material; the server **validates and stores it in
  `quiz_pending`** (`services/platform/quizme.py`), emits a `tool_call` with
  the student-facing card payload, and answers the model with a no-leak
  "question presented" result. The student answers on the card
  (`POST /chat/sessions/{id}/quiz-answer`): grades via exact choice match /
  the math **equivalence chain** / normalized text + pre-authorized accept
  variants, writes the verdict into `quiz_pending` (flag_modified — plain
  dict mutation does not dirty JSON columns), marks the asking message's
  card answered, stores the student's answer as a normal user message and
  enqueues the follow-up turn. Non-leak test asserts expected-answer
  bindings never appear in any gateway payload. Frontend: header toggle +
  interactive QuizCardView (choices or text input, verdict + expected
  answer). Backend 1016 + frontend 906 tests green.

- 2026-09-06 — **feat(forecast): plan 53-B — mastery-weighted exam-readiness
  estimate (ADR-118, plan 53 `dev/plans/53-…md`).** `exam_status` entries now
  carry a **readiness** estimate beyond the coverage bar:
  `course_readiness` (metrics.py) computes 100 × (0.5·coverage +
  0.35·mastery + 0.15·trend) where mastery is the answer-weighted accuracy
  across the course's weakness-matrix cells and trend compares the recent
  answer window against the prior one (↑ improving / → steady / ↓ declining
  at ±0.1); the up to 3 weakest in-scope concepts ride along as the evidence
  line. **Honesty gate:** fewer than 3 answered questions in scope →
  `readiness_state: "not_enough_data"`, no score. Surfaced as a
  ring + trajectory + weakest-concepts line on the Home exam card, a
  readiness chip on the Planner tab, and consumed by plan generation — when
  a forecast exists, practice items are scheduled on the first study day
  (bias-first). `ExamStatusOut`/generated types extended. Backend 1008 +
  frontend 904 tests green.

- 2026-09-06 — **feat(planner): plan 53-A — deadline-aware study planner
  (ADR-118, plan 53 `dev/plans/53-…md`).** The course workspace gains a
  root-only **Planner** tab backed by `plan_items` (migration 0054: profile/
  course/node placement via the composite FK so items follow node
  merge-delete and Promote-to-course, `kind`
  study|practice|review|milestone, `due_date`, `done_at`, `origin`
  manual|draft, `detail` evidence line; course purge removes items).
  **Generate plan** (`POST /courses/{id}/plan/generate`) is deterministic
  arithmetic, not AI: it reads the tree's engagement telemetry, paces
  untouched nodes across the days before `exam_date` (grouping when nodes
  outnumber days, spreading when days outnumber nodes), inserts weak-concept
  practice items from the diagnostics cells (accuracy < 0.7, n ≥ 3), weekly
  review milestones and an exam-day milestone — all as **draft** items;
  regeneration deletes only future undone drafts, never done or manual rows,
  and refuses missing/past exam dates (422). CRUD + check-off +
  drag-between-days / +1-day reschedule + manual items on the tab; Home's
  next-action card gains a **This week's plan** strip
  (`GET /plan/upcoming?days=7` across courses). Backend 1005 + frontend 902
  tests green.

- 2026-09-06 — **feat(research): plan 52-D — SEARCH/FETCH chat tools, web
  search provider setting, optional web-grounded genesis (ADR-136, plan 52
  `dev/plans/52-…md`) — plan 52 COMPLETE.** New `app/search/provider.py`
  adapter: Tavily-style `POST {base}/search` and SearXNG JSON API behind one
  interface (`flavor` + `base_url` in profile preferences, API key in the
  **keyring** (`search:api_key`), connect 1.5 s / read 10 s timeouts,
  `httpx` transport injectable for tests' socket guard). Settings →
  Providers gains a **Web search provider** card (`GET/PUT/DELETE
  /search-provider`; unassigned = feature absent). Chat tools: `SEARCH
  "query"` (budget 2/turn) and `FETCH <url>` (budget 1/turn, plan-47
  html→markdown converter, ≤4000 chars) join `CHAT_TOOL_CATALOG` — dispatched
  in **both engines** (legacy `answer_streaming` + graph `_tools` node with
  `search_used`/`fetch_used`/`search_sources` state), honest error lines when
  unconfigured, excerpts **never persisted** beyond the tool_calls summary
  (READ precedent), results rendered as **citation chips** (source domains) on
  the tool card. New **advisory** contract `sources_cited_when_search_used`
  (default chat contract + skill-contract builder): a turn that used
  SEARCH/FETCH but cites none of the retrieved URLs/domains logs an advisory,
  never blocks. Genesis step 1 gains a **Ground the outline in web sources**
  toggle (visible only with a provider): three deterministic queries feed
  excerpts into the outline prompt and committed courses carry a
  "Drafted with sources: …" description footer. Backend 997 + frontend 899
  tests green.

- 2026-09-06 — **feat(genesis): plan 52-B/C — course genesis + `lesson`
  compose kind (ADR-135, plan 52 `dev/plans/52-…md`).** "I want to learn X"
  now has a path. **Slice C** adds the `lesson` compose kind — an expository
  lesson (definitions → explanation → worked example → common pitfalls, no
  fabricated citations) whose source is **model knowledge**: the ContextSpec
  rides with whatever material the node has (none at genesis time); one-live
  artifact per (node, lesson), regeneration revises the user's edits; also
  selectable in the GenerateDialog compose builder. **Slice B** builds course
  genesis on it: `POST /courses/genesis/draft` (`{topic, level?}` — the
  existing `outline` task + `course.genesis` system prompt + deterministic
  validators, typed `GenesisDraftOut`, no DB writes) → `GenesisDialog`
  (`features/ai/`, 3 steps: topic form → editable outline → generation
  options) → `POST /courses/genesis` creates the course (`origin=genesis`,
  hidden=false), a node per chapter/section with the objective as `ai_hint`,
  estimates tasks = chapters × options and enforces the **budget cap**
  (`genesis_task_cap` profile preference, default 60, 422 over cap with an
  actionable message). Any selection enqueues one cancellable **`genesis`
  job** (`JobType.genesis`, `GenesisPayload`, handler in
  `pipelines/genesis.py`, registered in `main.py`): per depth-1 node it
  composes a lesson, generates a 5-question node-scoped quiz and 10
  flashcards (grounded in the lesson markdown when lessons ran), reports
  progress per task and **isolates failures** (per-task try/except +
  rollback + failed-activity cleanup; all-failed fails the job, partial
  failures complete with the failures in the report stage line). The
  frontend entry is a new Home **Generate a course from a topic** button
  beside the Scratchpad card; commit navigates into the new course.
  Backend 987 + frontend 898 tests green.

- 2026-09-06 — **feat(scratchpad): plan 52-A — a home for exploration +
  Promote-to-course (ADR-134, plan 52 `dev/plans/52-…md`).** The app is no
  longer closed-world: a per-profile **Scratchpad** system course
  (`origin='scratch'`, `hidden=true`, unique per profile via partial index
  `uq_courses_profile_scratch`; migration 0053 adds `courses.origin`
  (`manual|genesis|import|scratch`, `CourseOrigin` StrEnum) + `courses.hidden`)
  is seeded lazily by `ensure_scratch_course` and hosts notes, text/markdown
  files, chat uploads (the course-less upload fallback now prefers it over
  "Unsorted"), tutor chats and all AI actions. Hidden courses are excluded
  from `GET /courses` (`include_hidden=true` opts back in; new
  `GET /courses/scratchpad` ensures + returns it with a `content_count`, and
  `GET /courses/{id}` exists for hidden-course workspaces); the rail gains a
  Scratchpad row shown only when it has content, and Home gains an "Explore a
  topic" card. **Promote-to-course:** the scratch tree's node context menu
  gains **Promote to course…** → `POST /courses/{id}/nodes/{id}/promote`
  (409 outside the scratchpad) → wizard (title/subject/level/color) → a real
  course is created and `TreeService.move_subtree_to_course` re-parents the
  subtree across courses (SQLite `defer_foreign_keys` makes the composite-FK
  flip atomic), bulk-repoints every placement table (notes, quizzes,
  exercises, material/folder links, chat bindings), drops concept coverage for
  moved nodes (per-course graphs — stated honestly in the dialog), and moves
  scratch-exclusive folders/materials along (shared materials stay
  scratch-owned with a live cross-course link). Analytics scoping:
  `origin=scratch` courses are excluded from `exam_status` and from
  profile-wide `recommendations`/`answer_rows`/`due_cards_count`; streaks and
  per-course scores still count scratch activity. Docs: new
  `docs/usage/exploring.md`. Backend 981 + frontend 896 tests green.

- 2026-09-05 — **test(chat)+docs: open-issues accuracy pass.** The
  "flaky tests under parallel workers" open issue was stale — both chat-area
  races were already hardened on 2026-09-01 (30 s poll deadlines, DB polling
  after the WS event); the entry is now marked RESOLVED with the as-built
  detail. En route, `test_chat_turn_error.py`'s `wait_for_condition` gained a
  `describe` hook so its timeout error dumps the last observed event types /
  job status instead of a bare "condition never met" (sa-dev §4 rule), and the
  stale "Playwright e2e smoke is not scaffolded yet" line (superseded by plan
  50, 2026-09-02) was removed from Open issues.

- 2026-09-05 — **feat(library): text-file viewer tabs + material descriptions
  (user-requested).** Text/Markdown materials (kind `txt`/`md` or `text/*`
  mime) get their own viewer vocabulary — **Formatted** (rendered extraction) /
  **Raw text** (markdown source, monospace) / **Description** — replacing
  Extraction/Original/Side-by-side, which were redundant for authored files
  (original == content, and the stored blob goes stale after edits while Raw
  text reads the live extraction markdown); legacy `?tab=` values map onto
  Formatted. **Material descriptions are new:** nullable `materials.description`
  (migration 0052), set at creation (`POST /materials/text` gains
  `description`) and via `PATCH /materials/{id}` (title now optional, ≥1 field
  required — empty body 422; whitespace-only description normalizes to NULL;
  `MaterialsService.set_description`). Edit mode in the reader shows
  **Content / Description** tabs so the editor stays uncluttered (Save persists
  both; an unchanged description is never re-saved), and the new text/Markdown
  file dialog gains the same Content/Description tab pair — `onCreate`/`onSave`
  now take a single input object and the description rides along with Create
  and Save. OpenAPI schema + generated types refreshed. Backend 976 + frontend
  891 tests green.

- 2026-09-04 — **feat(content): version diffs for notes + material
  extractions (plan 58).** `app/services/content/diffs.py` (difflib unified
  diff, computed on read, nothing stored) + `GET /notes/{id}/versions/diff`
  and `GET /materials/{id}/extractions/diff` (`from`/`to` = version id or
  `current`, with ±line counts). Frontend: shared `DiffView` component;
  NoteHistoryDialog gains Compare mode (base/target selects + colored diff);
  new `ExtractionHistoryDialog` gives every material extraction versions +
  compare + restore (restore = append a new version, never destructive —
  same semantics as the existing mindmap history). Backend 969 + frontend
  879 tests green; alignment gate strict-clean. Decision recorded: snapshot
  versions + difflib diffs over embedding git (packaging + backup/restore
  burden; branch/merge unused) — see the local plan doc.

- 2026-09-04 — **feat(materials): plan 57 — batch "Save extracted text as
  material" from multi-select + filesystem-style derive naming (user-requested).**
  The derive verb (ADR-061) leaves the extraction view: right-clicking one or
  many selected files in the **Library** or a node's **Materials** tab offers
  **Save as material** / **Save N as materials** — one `POST /materials/derive`
  batch request (`{material_ids}` ≤200) returns per-source outcomes
  (`created`/`deduped`/`skipped` via new `DeriveOutcome` StrEnum); sources
  without a usable extraction are skipped and reported in a summary notice
  ("Saved N · K already existed · M skipped"), never blocking the batch; ingest
  jobs queue only for created materials. **Naming change:** derived materials
  drop the "(extracted)" suffix — `report.pdf` saves as `report.md`; the
  source's own title no longer collides (file-manager semantics), but any
  *other* material with the same name in the target folder does, resolved by a
  zero-padded counter (`report_01`, `report_02` — skipping taken numbers,
  296-char base cap); explicit Copy keeps `(copy)`. Menu gating is precise via
  a new `has_extraction` flag on `MaterialOut` (bulk-computed in `GET
  /materials` + detail) and `WorkspaceMaterialOut` (bulk-computed in
  `TreeService.workspace`). Backend 966 · frontend 872 tests green. Plan doc:
  `dev/projects/study-assistant/plans/57-batch-derive-from-selection.md` (local-only).
- 2026-09-04 — **feat(ai): chat-engine cutover — `SA_CHAT_ENGINE` defaults to
  `graph` (family AI-alignment Phase 8, plan 10).** Soaked first: a paced-token
  live-server soak (real uvicorn + WS client + staged mock provider covering
  plain turns, grammar CALC/PLOT rounds, reasoning deltas, NATIVE tool calls,
  stop-mid-stream, hint ladder, AG-UI mapping, checkpoint threads) found one
  real ordering gap — the round-end pending-line flush landed after the tool
  phase in graph mode — fixed with a deterministic `on_round_stream_end` hook
  from the agent node to the adapter pump; re-soak shows event-order parity
  with legacy modulo the ratified D1 (legacy's duplicate
  `stream_interrupted`). Default-engine coverage: the whole chat API test
  suite now runs the graph engine; legacy-streaming presentation tests are
  pinned to `chat_engine=legacy` explicitly. Playwright e2e smoke (incl. the
  chat tool-card spec) green on the default engine; backend 960 + frontend
  868 green; alignment gate strict-clean. `legacy` stays as rollback; the
  loop's deletion is a later release. D2 (`interrupt()` proposals) remains
  reserved by design.

- 2026-09-04 — **feat(ai): family event vocabulary + FlowStatusCard adoption
  (family AI-alignment migration Phase 6, plan 10 §6.2/6.3).** Chat turns
  emit the family event vocabulary additively alongside the legacy WS names
  (`app/agui/family.py` pure mapper over both engines; closed `FlowEvent`
  StrEnum; `stream_interrupted` unmapped by design). Frontend: dependency
  bump to `@neuronection/assistant-ui` ^0.18.0, new shim
  `components/ui/flow-status.ts`, and the shared editor's AI helper now
  renders the library `FlowStatusCard` — Transform → Review steps while
  streaming with Cancel (the duplicate footer Stop button is retired), and a
  retryable failed card on errors; chat keeps `TraceTimeline`. Backend 960 +
  frontend 868 tests green; alignment gate strict-clean.

- 2026-09-04 — **feat(ai): chat-turn graph engine — family AI-alignment
  migration Phase 5 (plan 10 §5.1–5.4, ADR-0008).** The chat turn now also
  runs as a checkpointed LangGraph `StateGraph` under `app/ai/graphs/`
  (`retrieve → contract_guard → agent_round ⇄ → validate_repair → finalize`),
  mirroring the legacy `answer_streaming` loop: native tools + in-memory
  degradation to the prompt grammar, per-kind tool budgets, contract repair
  loop, and **byte-identical WS event payloads + AG-UI mapping** (parity tests
  run both engines over the same scripted SSE model and compare normalized
  event sequences). Engine flag `SA_CHAT_ENGINE=legacy|graph` (default
  `legacy` — nothing flips; single-call `TaskRunner` tasks untouched).
  **5.1 scaffold**: deps `langgraph>=1.2,<2` + `langgraph-checkpoint-sqlite`
  **and** `-postgres`; dialect-picked checkpointer (`AsyncSqliteSaver` on
  `data_dir/checkpoints.db` desktop / `AsyncPostgresSaver` server) opened once
  in the app lifespan, `setup()` at boot, `thread_id` = chat session id,
  boot-time prune (`SA_CHECKPOINT_TTL_DAYS`, default 14; UUIDv6-id prefix
  comparison). **5.2**: custom StateGraph (not `create_agent` — dual tool
  modes + per-kind budgets + leak guards can't live in middleware); context,
  contracts, and persistence extracted into shared `ChatService`
  `prepare_turn_context`/`prepare_turn_contract`/`finalize_turn` helpers used
  by both engines (no drift by construction); `set_node_defaults` layers a
  hang-guard timeout + transport-leak-only retries over the gateway's own
  retry/fallback. **5.3 adapter**: `ChatTurnEngine` runs the graph on the app
  event loop from the `chat_turn` job handler and consumes the raw stable
  `astream(stream_mode=["updates", "messages"])` (integrator ruling
  2026-09-04 — v3 event streaming stays beta on the pinned langgraph; the
  swap is a one-file adapter change later) — `messages` chunks
  (`AIMessageChunk` text/reasoning) → throttled `stream_delta` with the legacy
  tool-line filter; `updates` flush the pending line at round boundaries and
  carry the finalize payload; `__interrupt__` is the reserved mapping point
  for future `interrupt()` proposals (fence proposals unchanged). **5.4 verify**: backend
  954 + frontend 866 tests green; `check-ai-alignment.sh --self` strict-clean
  (graphs under `app/ai/graphs/`, both checkpoint deps present). Local-only
  handoff: `dev/migration-notes.md`.

- 2026-09-03 — **feat(analytics): plan 51-H — adaptive difficulty via item-level
  Elo (C11, ADR-125).** Every graded **practice** answer now updates a
  deterministic Elo pair: the question's rating (seeded from its declared
  difficulty — `item_stats.rating`/`rating_count`, migration 0051) and the
  student's rating for each concept×skill cell the question touches (new
  `concept_skill_ratings` table — deliberately separate from
  `concept_skill_stats`, which materialize deletes-and-rebuilds). Math
  (`app/services/study/elo.py`): standard Elo with K declining 32→8 by attempt
  count, expected score from the rating difference, the answer's partial
  credit as the score. **Exam attempts are excluded server-side** at the
  submit hook. **Consumers**: focused quiz generation (concept/skill focus,
  no explicit difficulty) derives its difficulty target from the student's
  cell rating (`rating → difficulty`, recorded in `activity.config`); item
  analysis flags **`elo_outlier`** (`ItemFlag` + vocabulary + contract
  regenerated) when a question's live rating drifts ≥200 points from its
  declared-difficulty seed after ≥10 attempts — materialize applies it
  without touching `question.flag`. Migration 0051 (+ head-assertion bumps in
  the three migration tests); `docs/data-model.md` noted. Low-attempt
  questions keep wide uncertainty (no flag; targeting falls back to the
  requested difficulty); the plan's bank-UI "estimating" badge is deferred
  (no ratings surface in the bank yet). Tests: `test_elo.py` (7: seed/K/
  expected/difficulty/outlier math fixtures, exam exclusion server-side,
  practice flow updating item + student cell). Backend 945 · frontend 866
  green. Docs: features.md (C11 row), ai.md (targeting), data-model.md
  (0051). **This completes plan 51** — all seven slices (A/C/F/E/G/D/H) +
  ADRs 112–125 recorded.

- 2026-09-03 — **feat(quiz): plan 51-D — code-execution questions in Pyodide
  (C14, ADR-115).** New `code` question type: the student writes Python against
  **visible test cases** and the code runs **in Pyodide (CPython → WASM)
  inside the webview** — `pyodide` is a bundled dependency loaded as a lazy
  chunk (`pyodide.asm.wasm?url` → indexURL recipe), so execution works
  identically in the desktop shell and webapp mode, fully offline, and **no
  server-side execution path exists at all** (ADR-115). Harness
  (`lib/pyodideRunner.ts`): a module-level Pyodide singleton, the student's
  code `runPython`ed once, each test's call expression evaluated via
  `json.dumps(<call>)` with `setStdout` capture per case; comparison is
  in-page (number tolerance, line-wise string normalization) and the submitted
  payload (`{code, results: [{call, passed, output, stdout}]}`) is
  **re-verified by the backend** (`app/math/code.py`): each captured output is
  JSON-parsed and deterministically matched against the stored test
  (partial credit = passing cases, `failing_test` tag). **Schema**:
  `{starter_code, reference_solution, tests: [{call, expected,
  expected_stdout?}], timeout_ms?}` — validators require 1–10 tests with call +
  expected and a reference solution; `QuestionOut.input` exposes
  starter + tests + timeout (never the reference); caq export carries the
  tests but deliberately omits the reference solution. **Frontend**:
  `components/answers/CodeAnswer` (monospace editor pre-filled with the
  starter, *Run tests* with pending spinner, per-case pass/fail badges with
  "got:" output on failures), runner gating = completed run, practice builder
  gains a **Code** chip; i18n `quiz.code*`/`generate.questionType.code`.
  Known limits (documented): Pyodide stdlib + bundled packages only; the
  Pyodide interrupt buffer needs cross-origin isolation, so there is no hard
  timeout in v1 — an infinite loop freezes the tab (visible limitation, no
  server impact); reference-solution author validation runs in the same
  in-page path when authors preview (deferred UI affordance). Tests:
  `test_code_questions.py` (13: validation matrix, public-input hygiene incl.
  reference exclusion, grading incl. backend re-verification, stdout
  matching, string normalization, malformed payloads) + frontend
  `CodeAnswer.test.tsx` (4, mocked runner); GenerateDialog tests re-based on
  the 11th chip. Docs: features.md (C14 row), math-verification.md (grading
  table), ai.md (quizgen row).
...
- 2026-09-05 — **Fix: "There was an error during concurrent rendering" on
  opening the material editor (user report).** The AI helper's popover body
  was rendered by `FloatingPanel` on **every** host render even while closed,
  and the idle view (since the always-enabled-presets change) calls
  `editor.getText()` to decide whether presets can run — during the
  Edit-click mount, tiptap v3's `Editor` instance briefly exists **before
  its `schema` is initialized**, so `getText` → `getTextSerializersFromSchema`
  threw "Cannot read properties of null (reading 'nodes')" inside a
  concurrent render; React recovered synchronously but logged the error
  (intermittent — a create/init race, hence ~1-in-N openings). Two fixes:
  `FloatingPanel` now renders its children only while open (also stops the
  popover body — incl. the whole-document `getText()` — from re-running on
  every editor keystroke render), and `documentText()` treats a
  schema-less editor as empty. Diagnosed with an onRecoverableError probe +
  headless Chromium loop against the dev server (10/10 clean after, was
  ~1/5 before). Frontend suite green · backend untouched.

- 2026-09-05 — **feat: plan 59 slices A–E — the editor & AI-surface
  primitives are family library modules, adopted study-side
  (`@neuronection/assistant-ui@0.20.0`, ADR-133, cross-repo with career CV
  Studio as the second consumer).** Library (branch `feat/rich-text-editor`,
  5 commits + changesets → 0.20.0): `rich-text-editor` (controlled tiptap
  markdown primitive: toolbar groups + `toolbarExtra` app slot,
  `extensions` prop, `parseMarkdown`/`serializeMarkdown` fidelity hooks,
  caret-preserving echo-safe sync, SSR-safe), `dictation` (`useDictation`
  recording state machine + `DictationButton`/`DictationStrip`),
  `ai-text-transform` (`useAiTextTransform`: coalesced streaming, poll
  fallback, hard timeout, terminal-event guard), `text-diff-view` (the
  side-by-side diff + exported `computeLineDiff`/`wordSegments`) — each
  with tests (keyboard + axe), Ladle stories, docs pages, a11y rows and
  changesets; new deps `@tiptap/*` + `tiptap-markdown` +
  `@tanstack/react-virtual`. Study adoption: `AiHelperPopover` runs
  `useAiTextTransform` with the `ai-editor:{id}` transport, review diff and
  drawing-OCR diff render the library `TextDiffView` (i18next labels at
  call sites), `MarkdownEditor`/`ChatPanel` dictation runs `useDictation`
  through `lib/dictation.ts` (gateway transcribe + 409→`unassigned`);
  local `useEditorTransform`, dictation and lineDiff/TextDiffView
  implementations **deleted**; new shims `components/ui/{rich-text-editor,
  dictation,ai-text-transform,text-diff-view}.ts`. Verified via the tarball
  flow + against the published package (frontend 879 tests, lint,
  typecheck, build; e2e smoke unaffected). Remaining in plan 59:
  `MarkdownEditor` recomposition onto the primitive; career Slice F.

- 2026-09-05 — **Polish: the diff views get word-level highlighting, line
  numbers, change navigation and virtualization (user request).**
  `TextDiffView` upgraded: changed lines now also mark the **exact changed
  words** inside them (`lineDiff.wordSegments` — token-level LCS, whitespace
  noise excluded, `mark` spans over the `-`/`+` row backgrounds), each side
  shows **its own source line numbers** (independent original/suggested
  counters), the header gains **prev/next change navigation** with a
  "n/N" counter and an active-group tint that scrolls the view to each
  changed block, folds become **re-collapsible** ("Show less"), and long
  diffs are **virtualized** via `@tanstack/react-virtual` (measured dynamic
  rows, only visible rows in the DOM — whole-note diffs stay cheap). The OCR
  re-run diff inherits all of it (nav hidden there — single compact view).
  Frontend 904 tests (+6: word segments, numbering, navigation, reworked
  render test) · backend untouched.

- 2026-09-05 — **Feature: AI-suggestion diffs + OCR re-run comparison in the
  editor (user request).** Two surfaces now share a new client-side diff
  stack — `lib/lineDiff.ts` (LCS-based line diff: side-by-side aligned rows,
  context folding with expandable "N unchanged lines" blocks, `+N −M` stats)
  and `components/diff/TextDiffView.tsx` (gitlens-style two-column view with
  `-`/`+` gutters). **(1) The AI helper's review view is tabbed
  Diff/Formatted/Raw:** Diff is the default for transform runs and pins each
  changed block between folded unchanged context (the AI's changes are
  highlighted at every point); Formatted renders markdown; Raw is editable
  plain text (replacing the old preview/edit toggle). With a selection and a
  panel ≥600 px wide (ResizeObserver), a compact scrollable **Original pane**
  shows the source text left of the result on Formatted/Raw. Streaming deltas
  are now coalesced (~50 ms) in `useEditorTransform` so token floods can't
  stall rendering. **(2) "Run OCR again" compares old vs new:** the editor
  captures the previous transcript when re-OCR starts and, once the new text
  arrives over the `drawing_ocr` job flow, the inline transcript auto-opens a
  side-by-side diff with a "changed" badge and a Show-plain dismiss (shared
  via `DrawingDiffContext` into the drawing NodeView). Frontend 898 tests
  (+18: lineDiff engine, TextDiffView, review tabs/panes/write-default,
  DrawingBlock diff states, editor re-OCR flow) · backend untouched.

- 2026-09-05 — **UX: the AI helper's transform presets are always enabled — with
  no selection they run on the whole note (user request).** Presets previously
  disabled themselves without a selection and offered a separate "Apply to
  whole note" escape hatch; now every preset works unconditionally: a selection
  transforms just the selection (unchanged), no selection transforms the whole
  document text (≤12k chars, `mode: transform`), and an empty note keeps the
  presets disabled. The Context chip falls back to the whole note when nothing
  is selected. The popover states the scope up front — the section header
  switches between "Transform selection"/"Transform whole note" with a
  Selection/Whole-note pill — and the review view follows the run's scope:
  "Replace selection" for selection runs (range captured at run time, not at
  insert time), **"Replace note"** for whole-note runs via a new
  `replace-document` mode in `insertMarkdown`, and no replace verb for
  free-form write runs. Frontend 880 tests (+2 scope tests, reworked
  disabled-preset and whole-note tests) · backend untouched.
...
- **Flaky tests under parallel workers (observed 2026-08-28, twice) — RESOLVED
  (2026-09-01, see changelog "load-immune poll deadlines")**: both chat-area
  races were deadline blows, not product races — `test_chat_branches.py` poll
  helpers now use 30 s deadlines and `wait_until`'s timeout error dumps the
  last polled messages; `test_failed_turn_emits_turn_error_and_fails_job`
  polls the DB for the job row (never asserts right after the WS event) and
  its `wait_for_condition` timeout error dumps the last observed event types /
  job status (2026-09-05).
