...
- **The note header** (in the split view) holds an inline-editable title, tag
  chips (click **Add tag** to append one), an autosave status chip — click it
  to **Save now** — and the ✨ AI / **Make flashcards** actions; rare verbs
  (Print, Export .md, History, Delete) live in the ⋯ menu.
- A **fresh empty note** shows a quick-start strip (how to quote from the
  material, math + diagram syntax) that disappears once you write something.
- **Drag the divider** to resize; the split is remembered per course
  (30–70 %). Double-click the divider to reset to 50/50, or resize with the
  keyboard (←/→ while it's focused).
...
- **Save version now** checkpoints the current state immediately (bypasses the
  coalescing window).
- **Compare** picks any two snapshots (or the current text) and shows a
  unified, color-coded diff — green lines were added, red lines removed, with
  an `+added / −removed` line counter. This is how you answer "what exactly
...
## Snap a screen region into a note

Capture any part of your screen straight into the note you have open — a chart
from a PDF, a slide, a diagram from another app:

1. Press the **camera button in the editor toolbar** (or run **Snap a region
   into note** from the command palette, Ctrl/Cmd+K). Your browser asks which
   window, screen or tab to share.
2. **Drag a rectangle** over the part you want, then press **Use screenshot**.
   *Recapture* grabs a fresh frame; *Cancel* backs out.
3. A **preview popover** shows the capture. **Insert into note** places it at
   your cursor as an inline image — an **Undo** toast afterwards removes it
   again — while **Cancel** discards it and saves nothing.

With no note open, the palette flow saves the capture into a **new note** in
your current course (or the Scratchpad if none is selected). Snapshots are
plain images: the handwriting-OCR pass is skipped, and **Run OCR again** from
the drawing's ⋯ menu extracts their text on demand.
...
### Select anything, ask the AI

Selections aren't limited to text. Select a **diagram**, a **formula**, a
**table**, or drag across text and blocks **all together** — the helper
captures exactly that span as markdown and transforms it. A compact chip shows
what's included ("1 diagram · 2 formulas · 3 paragraphs"); click it to peek at
a short preview and the line count. When the selection includes a math or
diagram block, a **Try to fix** entry appears above the presets: it
self-diagnoses the block, tries an instant deterministic repair first, and
otherwise asks the AI with the renderer's error attached — the result reviews
as a diff and must render cleanly before it can replace the selection (if the
model rewrote it as something else entirely, it applies as plain markdown
instead, clearly labeled). Every transform — not just fixes — shows the diff
