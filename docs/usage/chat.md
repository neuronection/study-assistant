...
Every card shows **where it will act**: a row under the title names the target
file or note and its place in the course (e.g. *Material · integral-tricks.md ·
Analysis I › Chapter 3*). Edit/append proposals render the change as a
**formatted review** — headings and math render the way they'll read, with
changed paragraphs highlighted — and a **Raw** switch shows the exact markdown
source if you want to inspect it character by character. After approval,
material cards link straight to the created or edited material.
...
can use, one card per tool — expand a card to see what it does (with its scope),
its arguments (name, type, required), what it returns, and an example. Two groups:
...
Flip the **Quiz-me** toggle in the chat header and the tutor stops answering
and starts **testing you**: it asks exactly one question at a time (multiple
choice with buttons, or free text/math with an answer box), escalates as you
succeed, and briefly explains each miss.

Grading is **deterministic and server-side**: the tutor proposes the question
and its expected answer, but the app grades your reply (exact match, math
equivalence, or accepted variants) and only tells the tutor the verdict — the
tutor literally never sees the expected answer, so it cannot leak it. After
you answer, the verdict card shows whether you were right and the expected
answer. Every graded verdict **counts toward your daily answers** (streaks,
goal ring, XP) just like a quiz answer does. Turn Quiz-me off any time; it is
per-conversation.
