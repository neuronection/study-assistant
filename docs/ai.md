...
| **Quizgen** | `quizgen` | Blueprint → JSON questions → deterministic validators (metadata completeness, answer sanity, distractors ≠ answer via the equivalence chain; numberline answers validated as region payloads — domain bounds, interval ordering, at least one marker; table_fill grids validated per cell — alignment, kinds, parseable values, ≥1 fillable cell) → repair loop (max 2). Context via the Phase-10 resolver (scope/material opt-in-out/notes/concepts/hints). The `numberline` type (plan 51-A/ADR-112) and `table_fill` type (plan 51-F/ADR-123) let the model author richer answer surfaces whose grading is pure geometry / per-cell checkers — the model authors the stimulus and expected answers, never grades |
