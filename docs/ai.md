...
| **Quizgen** | `quizgen` | Blueprint → JSON questions → deterministic validators (metadata completeness, answer sanity, distractors ≠ answer via the equivalence chain; numberline answers validated as region payloads — domain bounds, interval ordering, at least one marker) → repair loop (max 2). Context via the Phase-10 resolver (scope/material opt-in-out/notes/concepts/hints). The `numberline` type (plan 51-A/ADR-112) lets the model author inequality/interval/solution-set questions whose grading is pure geometry — the model authors the stimulus and expected regions, never grades |
| **Exgen** | `exgen` | Multi-step exercise generation: JSON draft → validators (every expected answer parses via the chain, numeric tolerances sane, `expected_kind: numberline` answers validated as region payloads) → repair loop; refuses to persist an invalid exercise. Same pipeline powers **similar exercises** (isomorphic variant: same step structure, answers proven non-equivalent to the source) and **error-pattern drills** — patterns resolve from the course type's `error_patterns` taxonomy (plan 28/ADR-063), the prompt uses the course subject + pattern description/example (no hard-coded "calculus"). Context via the resolver |
