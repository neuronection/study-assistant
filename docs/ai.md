...
| **Quizgen** | `quizgen` | Blueprint → JSON questions → deterministic validators (metadata completeness, answer sanity, distractors ≠ answer via the equivalence chain; numberline answers validated as region payloads — domain bounds, interval ordering, at least one marker; table_fill grids validated per cell — alignment, kinds, parseable values, ≥1 fillable cell; composite parts validated per type with follow-through relations proven to parse, reference only prior parts, reproduce the declared value and survive a perturbed substitution) → repair loop (max 2). Context via the Phase-10 resolver (scope/material opt-in-out/notes/concepts/hints). The `numberline` (51-A/ADR-112), `table_fill` (51-F/ADR-123) and `composite` (51-E/ADR-122) types let the model author richer answer surfaces whose grading is deterministic — the model authors the stimulus and expected answers, never grades |
